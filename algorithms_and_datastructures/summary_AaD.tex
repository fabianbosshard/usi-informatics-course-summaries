...
%   \State $Q\gets V$ \Comment{min-priority queue by \attribute{key}}
%   \While{$Q\neq\emptyset$}
...
% \algobox{%
%   \begin{codebox}
%     \Procname{$\proc{MST-Prim}(G, u, w)$}
%     \li $T\gets(\emptyset,\emptyset)$
%     \li \For each vertex $v\in V(G)$
%     \li \Do $W[v]\gets\infty$
%     \li     $P[v]\gets \const{nil}$
%         \End
%     \li $W[u]\gets 0$
%     \li \While $V(T)\ne V(G)$
%     \li \Do find $u\not\in V(T)$ such that $W[u]$ is minimal
%     \li     $T\gets T\cup \{u\}$ \Comment add $u$ to $T$
%     \li     \For all $v\in \id{Adj}(u)\setminus V(T)$
%     \li     \Do \If $w(u,v) < W[v]$
%     \li         \Then $W[v]\gets w(u,v)$
%     \li               $P[v]\gets u$
%                 \End
%             \End
%         \End
...
%   \State Initialize $\attribnormal{v}{\delta}\gets\infty$, $\attrib{v}{\pi}\gets\nil$ for all $v\in V$
%   \State $\attribnormal{s}{\delta}\gets 0$
%   \State $Q\gets V$  \Comment{min-queue keyed by \attribnormal{·}{\delta}}
%   \While{$Q\neq\emptyset$}
%     \State $u\gets$ \Call{ExtractMin}{$Q$}
%     \ForAll{$(u,v)\in E$}
%       \If{$\attribnormal{v}{\delta}>\attribnormal{u}{\delta}+w(u,v)$}
%         \State $\attribnormal{v}{\delta}\gets\attribnormal{u}{\delta}+w(u,v)$
%         \State $\attrib{v}{\pi}\gets u$
%         \State \Call{DecreaseKey}{$Q,v,\attribnormal{v}{\delta}$}
%       \EndIf
%     \EndFor
%   \EndWhile
%   \State \Return $\bigl(\attribnormal{v}{\delta}\bigr)_{v\in V}$
% \EndFunction
% \end{algorithmic}
% \end{algorithm}
...
\section{Exam Exercises}\label{sec:exam_exercises}
