...
\midrule
accessing vertex $u$ &        \cellcolor{Green!25}\makecell{$O(1)$\\optimal} &                          \cellcolor{Green!25}\makecell{$O(1)$\\optimal}                    \\
\addlinespace[2pt]
iteration through $V$ &       \cellcolor{Green!25}\makecell{$\Theta(|V|)$\\optimal} &                   \cellcolor{Green!25}\makecell{$\Theta(|V|)$\\optimal}             \\
\addlinespace[2pt]
iteration through $E$ &       \cellcolor{Yellow!25}\makecell{$\Theta(|V|+|E|)$\\okay (not optimal)} &   \cellcolor{Red!25}\makecell{$\Theta(|V|^{2})$\\possibly very bad} \\
\addlinespace[2pt]
checking $(u,v)\in E$ &       \cellcolor{Red!25}\makecell{$O(|V|)$\\bad} &                              \cellcolor{Green!25}\makecell{$O(1)$\\optimal}                    \\
\addlinespace[2pt]
space complexity &            \cellcolor{Green!25}\makecell{$\Theta(|V|+|E|)$\\optimal} &               \cellcolor{Red!25}\makecell{$\Theta(|V|^{2})$\\possibly very bad} \\
\bottomrule
\end{tabular}
\end{table}
...
\Function{BFS}{$G=(V,E),\,s$}
  ...
  \State $\attrib{s}{color}\gets\gray$
