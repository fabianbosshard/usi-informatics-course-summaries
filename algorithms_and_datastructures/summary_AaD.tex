...
\begin{enumerate}
  ...
  An optimal solution to the problem contains within it optinmal solutions to subproblems.
\end{enumerate}
...
% \begin{example}[Activity Selection]
% For the unweighted version (all activities worth 1), selecting the compatible activity that \emph{finishes earliest} at every step is safe: by exchanging the first activity of any optimal schedule with the earliest-finish activity (slides~9--12), we obtain another optimal schedule of the same size, establishing the greedy-choice property. The sub-problem on the activities that start after the chosen one inherits the structure, satisfying optimal substructure. The resulting algorithm runs in $\Theta(n \log n)$ time after sorting by finish time.
% \[
%   \operatorname{OPT}(j) = \max\{v_j + \operatorname{OPT}(p(j)),\operatorname{OPT}(j-1)\} \qedhere
% \]
% \end{example}

% \begin{example}[Fractional Knapsack]
% Allowing fractional items, taking at each step the remaining item with highest value-to-weight ratio is optimal: 
% any deviation can be improved by swapping weight from a lower-ratio item to a higher-ratio one, contradicting optimality.
% \end{example}


% \begin{example}[0/1 Knapsack]
% With indivisible items the same rule fails, the greedy-choice property is violated.
% \end{example}


% \paragraph{Design hints.}
% \begin{itemize}
%   \item Inventing a tempting rule is usually easy; proving it correct is the hard part.
%   \item When correctness proofs become convoluted, the problem likely lacks the greedy-choice property—reconsider dynamic programming.
% \end{itemize}

\begin{remark}[Greedy versus Dynamic Programming]\label{rem:greedy_vs_dp}
...
% Dynamic programming systematically solves \emph{overlapping sub-problems} and stores their optimal values so that each is computed only once. 
% The term “programming” historically meant “tabular planning”.

% \begin{definition}[Principle of Optimality]\label{def:bellman_principle}
% An optimization problem satisfies the principle of optimality if an optimal solution \emph{contains} optimal solutions to its constituent sub-problems.
% \end{definition}

% DP applies when the principle of optimality holds but the greedy-choice property need not. Two implementation styles exist:
% \begin{itemize}
%   \item \textbf{Memoisation} (top-down): recurse as in a naïve exponential algorithm but cache every computed sub-problem.
%   \item \textbf{Tabulation} (bottom-up): explicitly fill a table in an order that guarantees dependencies are already solved.
% \end{itemize}

% \paragraph{Recipe.}
% \begin{enumerate}[label=\arabic*., leftmargin=*]
%   \item \emph{Characterise} an optimal solution by a function of one or more indices (the \emph{state}).
%   \item \emph{Derive} a recursion (\emph{transition}) on strictly smaller indices.
%   \item \emph{Compute} the states bottom-up (or memoise), remembering the choice that realised the optimum for reconstruction.
% \end{enumerate}

% \subsubsection*{Typical Sub-problem Structures}
% \begin{itemize}
%   \item \textbf{Prefix/suffix}: first $i$ elements (O(n) states), e.g.\ 0/1 Knapsack, Edit Distance.
%   \item \textbf{Sub-sequence/interval}: elements $i\ldots j$ (O(n²) states), e.g.\ Matrix-chain multiplication, Optimal BST.
% \end{itemize}

% \begin{example}[Weighted Activity Selection]
% Let the activities be indexed by increasing finish time $f_1<\dots<f_n$ and values $v_i$. For $j\in\{1,\dots,n\}$ define
% \[
%   \operatorname{OPT}(j)
%     = \max\{v_j + \operatorname{OPT}(p(j)),\operatorname{OPT}(j-1)\},
% \]
% where $p(j)=\max\{\,i<j\mid f_i\le s_j\}$ is the rightmost activity compatible with $j$. 
% Table filling takes $O(n\log n)$ time if the $p(j)$ values are pre-computed by binary search.
% \end{example}
% \begin{example}[Edit Distance (Levenshtein)]
% For strings $x[1..m]$ and $y[1..n]$ let $E(i,j)$ be the minimal edits turning $x[1..i]$ into $y[1..j]$. The recurrence is
% \[
%   E(i,j)
%     = \min
%       \begin{cases}
%         1 + E(i-1,j), & \text{delete},\\
%         1 + E(i,j-1), & \text{insert},\\
%         \mathbf{1}_{x_i\neq y_j} + E(i-1,j-1), & \text{match/replace}.
%       \end{cases}
% \]
% The $O(mn)$ table yields the classic $\Theta(mn)$ algorithm.
% \end{example}
% \begin{example}[Longest Increasing Sub-sequence (LIS)]
% For index $j$, let 
% \[
%   L(j)
%     = 1 + \max\{\,L(i)\mid i<j\wedge a_i<a_j\},
% \]
% with base $L(1)=1$. The straightforward DP is $O(n^2)$; with a balanced-search-tree trick it can be improved to $O(n\log n)$.
% \end{example}
% \begin{remark}\small
% While recursion \emph{suggests} itself from the mathematical recurrence, naïvely recursing without caching (e.g.\ Fibonacci) re-computes the same state exponentially many times—memoisation or tabulation are therefore indispensable.
% \end{remark}
...
The set P contains all problems that can be solved in polynomial time, i.e., the number of computational steps being at most a polynomial function of the input size. 
P is a subset of NP, containing all problems for which one can verify a given solution in polynomial time, whereas the solution may not be found in polynomial time.
...
\subsection*{294}
% \begin{algorithm}[h]
%   \caption{minimal number of edges to make a unditected graph connected}\label{alg:min_edges_connected}
%   \begin{algorithmic}[1]
%     \Function{MinimalAdditionalEdges}{$G$}
%       \State \Return \Call{CountComponents}{$G$} - 1
%     \EndFunction
%     \Function{CountComponents}{$G$}
%       \State $n_C \gets 0$ \Comment{number of components}
