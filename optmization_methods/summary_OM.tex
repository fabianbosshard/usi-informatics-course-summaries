...
\documentclass[11pt, headings=standardclasses, parskip=half]{scrartcl}
...
\begin{itemize}
  ...
  \item \(\mathcal{E}\) the set of equality constraints;
  \item \(\mathcal{I}\) the set of inequality constraints.
\end{itemize}


\begin{remark}\leavevmode
  \begin{itemize}
    \item We do not loose generality by restricting our analysis to minimisation problems, since maximizing \(f\) is \emph{equivalent to} minimising \(-f\).
...
% \section{Useful facts and formulas}

% % 1. Definition of $\mu$-strong convexity:
% \[
% f(y) \geq f(x) + \langle \nabla f(x),\, y-x \rangle + \frac{\mu}{2}\|y-x\|^2, \quad \forall x, y \in \mathbb{R}^n.
% \]

% % 2. Global minimizer and strong convexity:
% Let $x^*$ be the global minimizer of $f$, so that $\nabla f(x^*) = 0$. 
% Applying the definition with $y=x^*$, we have
% \[
...
% \]
% one can show (by completing the square or analyzing the corresponding quadratic) that
% \[
% \|\nabla f(x)\|^2 \geq 2\mu\bigl(f(x)-f(x^*)\bigr).
% \]

% % 4. Hessian lower bound:
% Strong convexity also implies that the Hessian satisfies
% \[
% \nabla^2 f(x) \succeq \mu I,
% \]
% which means that for every vector $z \in \mathbb{R}^n$,
% \[
% z^T \nabla^2 f(x) z \geq \mu \|z\|^2.
% \]
% Thus, every eigenvalue $\lambda$ of $\nabla^2 f(x)$ obeys
% \[
% \lambda \geq \mu > 0.
% \]

% % 5. Convexity of the modified function:
% Define
% \[
...
% \]
% which shows that $g(x)$ is convex.

% % 6. Invertibility of the Hessian and bounds on its inverse:
% Since all eigenvalues $\lambda$ of $\nabla^2 f(x)$ satisfy $\lambda \geq \mu > 0$, the Hessian is invertible.
% Moreover, the eigenvalues of $\left[\nabla^2 f(x)\right]^{-1}$ are $\frac{1}{\lambda}$, so
% \[
% 0 < \frac{1}{\lambda} \leq \frac{1}{\mu},
% \]
% and therefore,
% \[
% \left\|\left[\nabla^2 f(x)\right]^{-1}\right\| \leq \frac{1}{\mu}.
% \]
...
\begin{example}
The following minimisation problem is linear:
\[
...
An optimization problem is \emph{constrained} if \(\mathcal{E}\) OR \(\mathcal{I}\) are NOT empty. It is \emph{unconstrained} if both \(\mathcal{E}\) AND \(\mathcal{I}\) are empty.
...
\subsection{Global vs Local Optimization}
\label{subsec:global_local_optimization}
...
\label{def:global_minimizer}
A vector \(\vect{x}^{*}\in\Omega\) is a \emph{global minimizer} of \(f\) if
\[
f(\vect{x}^{*})\leq f(\vect{x}),\quad \forall\, \vect{x}\in\Omega.
\]
\end{definition}

\begin{definition}[Local Minimizer]
  \label{def:local_minimizer}
...
In unconstrained optimisation we can derive ``recipes'' to characterise solutions of our problem. These ``recipes'' are optimality conditions.
...
\]
In optimzation, we use Newton's method to find a stationary point (which \emph{may} be a local minimiser and \emph{is} a global minimiser if \(f\) is convex). Therefore, we use Newton's method to find a root of \(g=f'\), which corresponds to the update
\[
...
% above is my summary so far (actually I removed part of it to not hit your context window limit)
...
% so I specifically would like you to use the appropriate environments, acronyms and makros (for example $\vect{}$, $\matr{}$, theorem, algorithm environments etc....)
% needless to say, if there is e.g. an OCR recognized section number, DON'T include that in \section{}, as it will be generated by latex automatically...:
% please continue in the same style as the summary above, e.g. dont do numbering in the style 1-2-...-n but 1-...-n, so instead of $(x_1, x_2, ..., x_n)$ use $(x_1, \ldots, x_n)$ for example
% use hyperreferences to link to the appropriate sections, definitions, theorems, equations, algorithms etc. in the document. 
% do not use $\Bigl( \text{or} \Bigr)$, use $\left( \text{and} \right)$ instead.
% please use the same notation and style as in the summary above. use the algorithm environment for algorithms!
...
% To make Newton's method globally convergent from any starting point, we require more flexibility in the update rule. One common strategy is to take a smaller step in the same direction:
% \[
% \vect{x}^{(k+1)} = \vect{x}^{(k)} + \alpha \vect{d}_N,
% \]
% with \(\alpha \in (0,1)\) a step size. This is reminiscent of Gradient Descent, where the direction is \(\vect{d}_S = -\nabla f(\vect{x}^{(k)})\) instead of the Newton direction \(\vect{d}_N = -\left(\nabla^2 f(\vect{x}^{(k)})\right)^{-1} \nabla f(\vect{x}^{(k)})\).

% Both methods (and many of their variants) fall under the class of \emph{line search algorithms}, where determining a suitable step size is a key subroutine. A general line search algorithm has an update rule of the form:
% \[
...
% The term \emph{line search} specifically refers to the procedure of selecting a suitable \(\alpha^{(k)}\) given the current direction \(\vect{d}^{(k)}\). A minimal requirement for choosing \(\alpha^{(k)}\) is that it reduces the function value, i.e.
% \[
% f(\vect{x}^{(k+1)}) = f(\vect{x}^{(k)} + \alpha^{(k)} \vect{d}^{(k)}) < f(\vect{x}^{(k)}).
% \]

% This condition holds for sufficiently small \(\alpha^{(k)}\) if \(\vect{d}^{(k)}\) is a \emph{descent direction}, i.e.,
% \[
% \nabla f(\vect{x}^{(k)})^T \vect{d}^{(k)} < 0.
% \]

% Instead of performing an \emph{exact line search}, which solves the subproblem
% \[
% \min_{\alpha > 0} f(\vect{x}^{(k)} + \alpha \vect{d}^{(k)}),
% \]
% which is often expensive, practical methods rely on \emph{inexact line search}, such as the \emph{Wolfe conditions}.

% \subsection{Wolfe Conditions}
% \label{subsec:wolfe_conditions}

% The Wolfe conditions provide criteria for choosing a "good enough" step size \(\alpha^{(k)}\) to ensure global convergence.

% \paragraph{Definition (Armijo or Sufficient Decrease Condition).} Given a direction \(\vect{d}^{(k)}\), \(\alpha^{(k)}\) should satisfy:
% \[
% g(\alpha^{(k)}) = f(\vect{x}^{(k)} + \alpha^{(k)} \vect{d}^{(k)}) \le f(\vect{x}^{(k)}) + \eta \alpha^{(k)} \nabla f(\vect{x}^{(k)})^T \vect{d}^{(k)},
% \]
% where \(\eta \in (0,1)\) is a small relaxation parameter (e.g., \(\eta = 10^{-4}\)).

% \paragraph{Definition (Curvature Condition).} To prevent \(\alpha^{(k)}\) from being too small:
% \[
% \nabla f(\vect{x}^{(k)} + \alpha^{(k)} \vect{d}^{(k)})^T \vect{d}^{(k)} \ge \bar{\eta} \nabla f(\vect{x}^{(k)})^T \vect{d}^{(k)},
% \]
% with \(\bar{\eta} \in (\eta, 1)\), typically \(\bar{\eta} \in [0.6, 0.9]\).

% \subsection{Backtracking Line Search}
% \label{subsec:backtracking_line_search}

% The backtracking line search algorithm starts from an initial step size \(\bar{\alpha} > 0\) and reduces it until the Armijo condition is satisfied.

% \begin{algorithm}[H]
%   \caption{Backtracking Line Search}
%   \label{alg:backtracking_line_search}
%   \begin{algorithmic}[1]
%     \State \textbf{Input:} Point \(\vect{x}^{(k)}\), descent direction \(\vect{d}^{(k)}\)
%     \State \textbf{Parameters:} Initial step size \(\bar{\alpha} > 0\), relaxation parameter \(\eta \in (0,1)\), shrinkage factor \(\rho \in (0,1)\)
%     \State Set \(\alpha^{(k)} \gets \bar{\alpha}\)
%     \While{\(f(\vect{x}^{(k)} + \alpha^{(k)} \vect{d}^{(k)}) > f(\vect{x}^{(k)}) + \eta \alpha^{(k)} \nabla f(\vect{x}^{(k)})^T \vect{d}^{(k)}\)}
%       \State \(\alpha^{(k)} \gets \rho \alpha^{(k)}\)
%     \EndWhile
...
%   Suppose:
%   \begin{itemize}
%     \item \(f: \mathbb{R}^n \to \mathbb{R}\) is continuously differentiable, L-smooth, and bounded from below,
%     \item Each \(\vect{d}^{(k)}\) is a descent direction, and \(\alpha^{(k)}\) satisfies the Wolfe conditions.
%   \end{itemize}
%   Then:
%   \[
%   \sum_{k \ge 0} \cos^2(\theta^{(k)}) \|\nabla f(\vect{x}^{(k)})\|^2 < \infty,
%   \]
%   where \(\cos \theta^{(k)} = \frac{\nabla f(\vect{x}^{(k)})^T \vect{d}^{(k)}}{\|\nabla f(\vect{x}^{(k)})\| \|\vect{d}^{(k)}\|}\).
% \end{theorem}

% This implies that if \(\cos \theta^{(k)}\) is bounded away from 0, then \(\|\nabla f(\vect{x}^{(k)})\| \to 0\).

% \paragraph{Special Cases:}
% \begin{itemize}
%   \item \textbf{Gradient Descent:} \(\vect{d}^{(k)} = -\nabla f(\vect{x}^{(k)})\) \(\Rightarrow \cos \theta^{(k)} = -1\).
%   \item \textbf{Newton-type methods:} \(\vect{d}^{(k)} = -[\matr{H}^{(k)}]^{-1} \nabla f(\vect{x}^{(k)})\), where \(\matr{H}^{(k)} \succ 0\), implies
%   \[
%   \cos \theta^{(k)} \ge \frac{\lambda_{\min}(\matr{H}^{(k)})}{\lambda_{\max}(\matr{H}^{(k)})} = \frac{1}{\kappa(\matr{H}^{(k)})}.
%   \]
% \end{itemize}

% Thus, if \(\kappa(\matr{H}^{(k)})\) is bounded, \(\cos \theta^{(k)}\) is bounded away from 0, ensuring convergence.

% \section{Variations of Newton's Method}
% \label{sec:variations_newton}

% Despite its strong convergence properties, Newton's method has two main drawbacks:
% \begin{itemize}
%   \item It requires the Hessian matrix \(\nabla^2 f(\vect{x}^{(k)})\) to be positive definite.
%   \item Computing second-order derivatives is computationally expensive.
% \end{itemize}

% \subsection{Newton with Hessian Correction}
% \label{subsec:hessian_correction}

% If the Hessian \(\matr{H}^{(k)} = \nabla^2 f(\vect{x}^{(k)})\) is not positive definite, the Newton direction
% \[
% \vect{d}^{(k)} = -[\matr{H}^{(k)}]^{-1} \nabla f(\vect{x}^{(k)})
% \]
% may not be a descent direction. A remedy is to correct the Hessian by adding a multiple of the identity:
% \[
% \matr{H}^{(k)} + \lambda \matr{I}, \text{ with } \lambda > 0,
% \]
% until the matrix becomes positive definite. One can test this via the Cholesky decomposition:
% \[
% \matr{H} = \matr{L} \matr{L}^{T}.
% \]
% If the decomposition fails, increase \(\lambda\) until it succeeds.

% \begin{algorithm}[H]
%   \caption{Hessian Correction via Cholesky Decomposition}
%   \label{alg:hessian_correction}
%   \begin{algorithmic}[1]
%     \State \textbf{Input:} Hessian \(\matr{H} = \nabla^2 f(\vect{x})\)
%     \State \textbf{Parameters:} Initial \(\bar{\lambda} > 0\), growth factor \(c > 1\)
%     \If{Cholesky succeeds on \(\matr{H}\)}
%       \State \Return Cholesky factor \(\matr{L}\)
%     \Else
%       \State Set \(\lambda \gets \bar{\lambda}\)
%       \While{Cholesky fails on \(\matr{H} + \lambda \matr{I}\)}
%         \State \(\lambda \gets c \lambda\)
%       \EndWhile
%       \State \Return Cholesky factor of corrected Hessian
%     \EndIf
%   \end{algorithmic}
% \end{algorithm}

% \subsection{Quasi-Newton Methods}
% \label{subsec:quasi_newton}

% Quasi-Newton methods build Hessian approximations using gradients only. The goal is to satisfy the \emph{secant equation}:
% \[
% \tilde{\matr{H}}^{(k+1)} \vect{s}^{(k)} = \vect{y}^{(k)},
% \]
% with:
% \[
% \vect{s}^{(k)} = \vect{x}^{(k+1)} - \vect{x}^{(k)}, \quad \vect{y}^{(k)} = \nabla f(\vect{x}^{(k+1)}) - \nabla f(\vect{x}^{(k)}).
% \]
% A sufficient condition for positive definiteness is:
% \[
% (\vect{s}^{(k)})^T \vect{y}^{(k)} > 0.
% \]

% \paragraph{DFP Method.} Updates the Hessian approximation directly:
% \[
% \tilde{\matr{H}}^{(k+1)} = \left(\matr{I} - \frac{\vect{y}^{(k)} (\vect{s}^{(k)})^T}{\rho^{(k)}}\right)^T \tilde{\matr{H}}^{(k)} \left(\matr{I} - \frac{\vect{y}^{(k)} (\vect{s}^{(k)})^T}{\rho^{(k)}}\right) + \frac{\vect{y}^{(k)} (\vect{y}^{(k)})^T}{\rho^{(k)}},
% \]
% where \(\rho^{(k)} = (\vect{y}^{(k)})^T \vect{s}^{(k)}\).

% \paragraph{BFGS Method.} Updates the inverse Hessian approximation:
% \[
...
% This allows computing descent directions without second-order derivatives:
% \[
% \vect{d}^{(k+1)} = -\matr{B}^{(k+1)} \nabla f(\vect{x}^{(k+1)}).
% \]

% Both methods require an initial guess (e.g., \(\matr{B}^{(0)} = \gamma \matr{I}\)).

% \bigskip

% We conclude that variations of Newton's method, such as line search, Hessian correction, and Quasi-Newton methods, are essential for achieving global convergence and reducing computational cost in practice.
...
\begin{algorithmic}[1]
    ...
    \While{\(\|\nabla f(\vect{x}^{(k)})\| > \epsilon\) and \(k \leq k_{\max}\)}
        ...
        \State Update: \(\vect{x}^{(k+1)} = \vect{x}^{(k)} + \alpha^{(k)}\,\vect{d}^{(k)} \text{,}\)
...
Recall Equation~\eqref{eq:form_optimization_problem}, the general formulation of a constrained optimization problem, as well as Equation~\eqref{eq:feasible_set}, the definition of the feasible set. 

\begin{definition}[Active Set]\label{def:active_set}
  The active set \(\mathcal{A}\) at any feasible \(\vect{x}\) consists of the equality constraint indices from \(\mathcal{E}\) together wiwth the indices of the inequality constraints \(i\) for which \(c_i(\vect{x}) = 0\); that is,
  \[
  \mathcal{A} = \mathcal{E} \cup \{i \in \mathcal{I} \mid c_i(\vect{x}) = 0\} \text{.}
  \]
...
\begin{itemize}[nosep, itemsep=0pt, parsep=0pt, before={\parskip=0pt}]
  ...
  \item Do you know how to recognise that a point is a local minimiser?
  \item Do you know what is a descent direction?
  \item Do you know what is a quadratic function and in which cases it has a global minimiser?
...
%     \min_{x} f(x),
%     \]
%     a necessary condition for \(x^*\) to be a local minimiser is 
%     \[
%     \nabla f(x^*) = 0.
%     \]
%   \item \textbf{Second-Order Necessary Condition:} If \(\nabla f(x^*) = 0\), then a necessary condition is that the Hessian is positive semidefinite:
%     \[
%     \nabla^2 f(x^*) \succeq 0.
%     \]
...
% \subsubsection*{Directional Derivative and Descent Directions}
% \begin{itemize}[leftmargin=2em]
%   \item \textbf{Directional Derivative:} For a function \(f: \mathbb{R}^n \to \mathbb{R}\), the directional derivative at \(x\) in the direction \(d\) is defined as
%     \[
%     f'(x; d) = \lim_{t \to 0} \frac{f(x + t d) - f(x)}{t},
%     \]
%     provided the limit exists.
%   \item \textbf{Descent Direction:} A vector \(d\) is a descent direction at \(x\) if
%     \[
%     \nabla f(x)^T d < 0.
%     \]
%     This means that for sufficiently small \(t > 0\), we have \(f(x + t d) < f(x)\).
% \end{itemize}

% \subsubsection*{Convex, Strongly Convex, Lipschitz, and Smooth Functions}
% \begin{itemize}[leftmargin=2em]
%   \item \textbf{Convex Function:} A function \(f\) is convex if for all \(x, y\) and \(\lambda \in [0,1]\),
%     \[
...
%     \|\nabla f(x) - \nabla f(y)\| \le L \|x-y\|.
%     \]
%   \item \textbf{Smooth Function:} A function is called \(L\)-smooth if it is differentiable and its gradient is Lipschitz continuous with constant \(L\). In particular,
%     \[
%     f(y) \le f(x) + \nabla f(x)^T (y-x) + \frac{L}{2}\|y-x\|^2.
%     \]
% \end{itemize}

% \subsubsection*{Properties of Multivariate Quadratic Functions}
% \begin{itemize}[leftmargin=2em]
%   \item \textbf{General Form:} A quadratic function in \(\mathbb{R}^n\) can be written as
%     \[
%     f(x) = \frac{1}{2} x^T A x + b^T x + c,
%     \]
%     where \(A\) is a symmetric matrix, \(b\) is a vector, and \(c\) is a scalar.
%   \item \textbf{Convexity:} The function is convex if \(A \succeq 0\) (i.e., \(A\) is positive semidefinite) and strictly convex if \(A \succ 0\) (i.e., positive definite).
%   \item \textbf{Global Minimiser:}
%     \begin{itemize}
%       \item If \(A \succ 0\), then the unique global minimiser is
%         \[
%         x^* = -A^{-1}b.
%         \]
%       \item If \(A \succeq 0\) but is singular, a global minimiser exists if and only if \(b\) lies in the range (column space) of \(A\); however, the minimiser may not be unique.
%     \end{itemize}
% \end{itemize}

% \subsubsection*{Wolfe Conditions}
% The Wolfe conditions are used in line search methods to choose a step size \(\alpha\). They consist of:
% \begin{itemize}[leftmargin=2em]
...
% \subsubsection*{Recognising Convex and Smooth Functions}
% \begin{itemize}[leftmargin=2em]
%   \item \textbf{Convex Functions:} Check if the line segment between any two points on the graph lies above the graph. For differentiable functions, if the second derivative (or Hessian) is nonnegative (or positive semidefinite), the function is convex.
%   \item \textbf{Smooth Functions:} These are continuously differentiable functions. In optimization, a smooth function often also has a Lipschitz continuous gradient.
% \end{itemize}

% \subsubsection*{Recognising a Local Minimiser}
% A point \(x^*\) is a local minimiser if:
% \begin{itemize}[leftmargin=2em]
%   \item \textbf{First-Order Test:} \(\nabla f(x^*) = 0\).
%   \item \textbf{Second-Order Test:} The Hessian \(\nabla^2 f(x^*)\) is positive definite (or at least positive semidefinite, with additional conditions to rule out saddle points).
%   \item Additionally, there exists a neighborhood \(U\) of \(x^*\) such that \(f(x^*) \le f(x)\) for all \(x \in U\).
% \end{itemize}

% \subsubsection*{Descent Direction}
% A descent direction \(d\) at a point \(x\) satisfies:
% \[
% \nabla f(x)^T d < 0,
% \]
% which ensures that moving a small distance in the direction \(d\) decreases the function value.

% \subsubsection*{Quadratic Function and Global Minimiser}
% \begin{itemize}[leftmargin=2em]
...
%     \begin{itemize}
%       \item If \(A \succ 0\), the function is strictly convex and the unique global minimiser is
%         \[
%         x^* = -A^{-1}b.
%         \]
%       \item If \(A \succeq 0\) (and not positive definite), a global minimiser exists if \(b\) is in the range of \(A\); if not, the function is unbounded below. In this case, the minimiser may not be unique.
%     \end{itemize}
% \end{itemize}

% \subsubsection*{Gradient Descent and Newton's Algorithms}
% \begin{itemize}[leftmargin=2em]
%   \item \textbf{Gradient Descent:} An iterative method where the update is
%     \[
%     x_{k+1} = x_k - \alpha_k \nabla f(x_k),
%     \]
%     with \(\alpha_k\) as the step size (which can be determined by a line search satisfying Wolfe or Armijo conditions).
%   \item \textbf{Newton's Method:} An iterative method that uses second-order information:
%     \[
%     x_{k+1} = x_k - \left[\nabla^2 f(x_k)\right]^{-1} \nabla f(x_k).
%     \]
%     Newton's method can have quadratic convergence near the minimiser if the Hessian is positive definite and the starting point is sufficiently close.
% \end{itemize}

% \subsubsection*{Applying the Wolfe Conditions}
% To apply the Wolfe conditions during a line search:
% \begin{enumerate}[leftmargin=2em]
%   \item Choose an initial step size \(\alpha\) and compute the trial point \(x_k + \alpha d_k\).
...
%     \nabla f(x_k + \alpha d_k)^T d_k \ge c_2 \nabla f(x_k)^T d_k.
%     \]
%   \item If both conditions are met, accept \(\alpha\); otherwise, adjust \(\alpha\) (typically reduce it) and repeat.
% \end{enumerate}

% \subsubsection*{Positive-Semidefiniteness and Positive-Definiteness of the Zero Matrix}
% For a square zero matrix \(\mathbf{0}\) (all entries are 0):
% \begin{itemize}[leftmargin=2em]
%   \item \textbf{Positive-Semidefinite:} Yes, because for any vector \(x\),
%     \[
%     x^T \mathbf{0} \, x = 0 \ge 0.
%     \]
%   \item \textbf{Positive-Definite:} No, since for any nonzero \(x\),
%     \[
%     x^T \mathbf{0} \, x = 0 \quad (\text{not strictly greater than 0}).
%     \]
% \end{itemize}

% \subsubsection*{Analysis of the Given Matrix}
% Consider the matrix
% \[
...
%     \]
%   \item \textbf{Condition Number:} The 2-norm condition number is the ratio of the largest to smallest eigenvalue:
%     \[
%     \kappa(\mathbf{A}) = \frac{2.5}{0.5} = 5.
%     \]
%   \item \textbf{Spectral Norm:} The spectral norm is the largest absolute eigenvalue:
%     \[
%     \|\mathbf{A}\|_2 = 2.5.
%     \]
%   \item \textbf{Inverse of \(\mathbf{A}\):} Since \(\mathbf{A}\) is diagonal, its inverse is given by the reciprocals of the diagonal entries:
%     \[
...
%     \end{pmatrix}.
%     \]
%   \item \textbf{Eigenvalues of \(\mathbf{A}^{-1}\):} They are the reciprocals of the eigenvalues of \(\mathbf{A}\):
%     \[
%     1,\quad 0.4,\quad 2.
%     \]
% \end{itemize}
...
\begin{description}[leftmargin=1.5cm, style=nextline]
  ...
  \begin{itemize}[leftmargin=2em]
    \item \textbf{First-Order Necessary Condition:} For an unconstrained problem 
      ...
      \]
      a necessary condition for \(\vect{x}^*\) to be a local minimiser is 
      \[
      \nabla f(\vect{x}^*) = 0.
      \]
    ...
    \item \textbf{Second-Order Sufficient Condition:} If \(\nabla f(\vect{x}^*) = 0\) and 
      ...
      \]
      then \(\vect{x}^*\) is a strict local minimiser.
  \end{itemize}
  ...
  \begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{Global Minimiser:}
      \begin{itemize}
        \item If \(\matr{A} \succ 0\), then the unique global minimiser is
          \[
          \vect{x}^* = -\,\matr{A}^{-1}\,\vect{b}.
          \]
        \item If \(\matr{A} \succeq 0\) but is singular, a global minimiser exists if and only if \(\vect{b}\) lies in the range (column space) of \(\matr{A}\); however, the minimiser may not be unique.
      \end{itemize}
  \end{itemize}
  ...
  \item[Q7: How do you recognise that a point is a local minimiser?]
  \textbf{A:} A point \(\vect{x}^*\) is a local minimiser if:
  \begin{itemize}[leftmargin=2em]
    \item \textbf{First-Order Test:} \(\nabla f(\vect{x}^*) = 0\).
  ...
  \item[Q9: What is a quadratic function and in which cases does it have a global minimiser?]
  \textbf{A:}
  \begin{itemize}[leftmargin=2em]
    \item \textbf{Quadratic Function Form:}
      \[
    ...
    \item \textbf{Global Minimiser Existence:}
      \begin{itemize}
        \item If \(\matr{A} \succ 0\), the function is strictly convex and the unique global minimiser is
          \[
          \vect{x}^* = -\,\matr{A}^{-1}\,\vect{b}.
          \]
        \item If \(\matr{A} \succeq 0\) (but not positive definite), a global minimiser exists if \(\vect{b}\) lies in the range of \(\matr{A}\); otherwise, the function is unbounded below. The minimiser may not be unique in that case.
      \end{itemize}
  \end{itemize}
  ...
  \begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{Newton's Method:} Update using second-order information:
      ...
      \]
      This method exhibits quadratic convergence near the minimiser provided the Hessian is positive definite and the initial guess is close enough.
  \end{itemize}
