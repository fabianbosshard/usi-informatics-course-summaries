...
\begin{tikzpicture}%[scale=0.3] %scale does not work here
  ...
  \begin{axis}[
    scale=0.8,
      ...
      xshift=1.5cm,                      % manual separation
      width=9cm, height=6cm,
      axis lines=middle,
      axis line style={->},
      xlabel={\footnotesize$\alpha$},
...
\definecolor{vecPink}{RGB}{255, 188, 250}
\colorlet{vecCyan}{cyan!20!white}
\colorlet{vecBlueTemp}{blue!60!cyan}
\colorlet{vecBlue}{vecBlueTemp!40!white}
...
\def\labx{3}
\def\laby{2.5}
% Four XY corners of the grey plane patch (clockwise)
\def\pax{4} \def\pay{6}
\def\pbx{6} \def\pby{-1}
\def\pcx{-1} \def\pcy{-1}
\def\pdx{-1} \def\pdy{6}
% Spannvektoren u, v
\def\uxdir{2}  \def\uydir{-4}
\def\vxdir{4}  \def\vydir{-2}
% Arrow/scale knobs
\def\nshort{0.8}   % |n_0|
\def\nlong{2.4}   % |n|
\def\vecscale{2.5}% |u|, |v|
% -------------------------------------------------------------
...
% Aufpunkt Q  (compute z so it lies on the plane)
...
% --- Custom in‑plane unit vectors u and v --------------------
% First choose raw XY direction; then solve z so n·u = 0 ⇒ z = -(nx x + ny y)/nz
% Finally normalise each to unit length.
\pgfmathsetmacro{\uzraw}{-(\nx*\uxdir + \ny*\uydir)/\nz}
\pgfmathsetmacro{\uMag}{sqrt(\uxdir*\uxdir + \uydir*\uydir + \uzraw*\uzraw)}
\pgfmathsetmacro{\ux}{\uxdir/\uMag}
\pgfmathsetmacro{\uy}{\uydir/\uMag}
\pgfmathsetmacro{\uz}{\uzraw/\uMag}

\pgfmathsetmacro{\vzraw}{-(\nx*\vxdir + \ny*\vydir)/\nz}
\pgfmathsetmacro{\vMag}{sqrt(\vxdir*\vxdir + \vydir*\vydir + \vzraw*\vzraw)}
\pgfmathsetmacro{\vx}{\vxdir/\vMag}
\pgfmathsetmacro{\vy}{\vydir/\vMag}
\pgfmathsetmacro{\vz}{\vzraw/\vMag}

% Patch‑corner z values (solve plane eqn)
