...
\begin{itemize}
  \item $f$ and $c_i$'s are \emph{affine} functions: \nameref{sec:linear_programming} problem (Section~\ref{sec:linear_programming})
...
\begin{definition}[Set of linearized feasible directions]\label{def:linearized_feasible_directions}
  ...
  \[
    \mathcal{F}(\vect{x}) := \{ \vect{d}\in\R^{n} \mid \nabla c_{i}(\vect{x})^{\top}\vect{d}=0 \; \forall i\in\mathcal{E} \land \nabla c_{i}(\vect{x})^{\top}\vect{d}\ge 0 \; \forall i\in\mathcal{I}\cap\mathcal{A}(\vect{x}) \}
    \qedhere
  \]
\end{definition}
...
\begin{lemma}[Fundamental Necessary Condition]\label{lem:fundamental_condition}
  ...
  \]
  \textcolor{red}{for all feasible directions \(\vect{d}\in\mathcal{F}(\vect{x}^{\ast})\)}.
  Since we would need to examine every \(\vect{d}\in\mathcal{F}(\vect{x}^{\ast})\), this condition is not very practical.
\end{lemma}

\begin{theorem}[\gls{kkt} conditions]\label{thm:KKT}
...
\begin{subequations}\label{eq:KKT}
  \begin{align}
    ...
    c_{i}(\vect{x}^{\ast}) &= 0,                      \quad \forall i\in\mathcal{E}                             &&\text{(primal feasibility)}                \label{eq:KKT_equal_feas} \\
    c_{i}(\vect{x}^{\ast}) &\ge 0,                    \quad \forall i\in\mathcal{I}                             &&\text{(primal feasibility)}                \label{eq:KKT_inequal_feas} \\
    \lambda_{i}^{\ast} &\ge 0,                        \quad \forall i\in\mathcal{I}                             &&\text{(dual feasibility)}                  \label{eq:KKT_dual_feas} \\
    \lambda_{i}^{\ast} c_{i}(\vect{x}^{\ast}) &= 0,   \quad \forall i\in\mathcal{E}\cup\mathcal{I}              &&\text{(complementary slackness)}           \label{eq:KKT_comp_slack} % complementarity slackness
  \end{align}
\end{subequations}
...
\[
\nabla_{\vect{x}} \mathcal{L}(\vect{x}^{\ast},\vect{\lambda}^{\ast}) = \vect{0} 
\]
...
\begin{lemma}[Farkas]\label{lem:Farkas}
  ...
  K : = \{ \matr{B} \vect{y} + \matr{C} \vect{w} \mid \vect{y} \in \R^m_{\ge 0}, \vect{w}\in\R^{p} \} 
\end{equation}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item \( \exists \vect{d}\in\R^{n} \mid \vect{g}^{\top} \vect{d} < 0, \matr{B}^{\top} \vect{d} \ge \vect{0}, \matr{C}^{\top} \vect{d} = \vect{0} \) \label{item:Farkas_hyperplane}
  \end{enumerate}
  Geometrically \ref{item:Farkas_hyperplane} says that if \(\vect{g} \notin K\), we can find a hyperplane with normal vector \(\vect{d}\) separating \(\vect{g}\) from the cone \(K\) such that \(\vect{g}\) lies on the opposite side of \(\vect{d}\).
\end{lemma}
...
\begin{definition}[Critical cone]\label{def:critical_cone}
  For a KKT point \( (\vect{x},\vect{\lambda}) \), the \emph{critical cone} is
  \[
    \mathcal{C}(\vect{x},\vect{\lambda}) := \{ \vect{d}\in\mathcal{F}(\vect{x}) \mid \nabla c_{i}(\vect{x})^{\top} \vect{d} = 0\; (i\in\mathcal{I}\cap\mathcal{A}(\vect{x}), \lambda_{i}>0) \}.
  \]
\end{definition}

\begin{definition}[Lagrangian Hessian]\label{def:Lagrangian_Hessian}
  \[
    \nabla^{2}_{\vect{x}\vect{x}} \mathcal{L}(\vect{x},\vect{\lambda}) := \nabla^{2} f(\vect{x}) - \sum_{i\in\mathcal{E}\cup\mathcal{I}} \lambda_{i} \nabla^{2} c_{i}(\vect{x}).
  \]
\end{definition}

\begin{theorem}[Second-order necessary condition]\label{thm:SONC}
  Let \( \vect{x}^{\ast} \) be a local minimizer satisfying \nameref{def:constraint_qualification_condition} and let \( \vect{\lambda}^{\ast} \) be the unique multiplier vector from Theorem~\ref{thm:KKT}. Then
  \[
...
\begin{theorem}[Second-order sufficient condition]\label{thm:SOSC}
  Suppose that \( (\vect{x}^{\ast},\vect{\lambda}^{\ast}) \) satisfies the KKT conditions. If
  \[
