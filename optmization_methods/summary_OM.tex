...
\begin{subequations}\label{eq:KKT}
  \begin{align}
    ...
    &\qquad\qquad\qquad&\lambda_{i}^{\ast} c_{i}(\vect{x}^{\ast}) &= 0,   \quad \forall i\in\mathcal{E}\cup\mathcal{I}              &&\text{(complementary slackness)}           \label{eq:KKT_comp_slack} % complementarity slackness
  \end{align}
\end{subequations}
...
% A decrease is not possible, because from \eqref{eq:KKT_stationarity_active} we have
% \begin{equation}\label{eq:KKT_objective_gradient}
% \nabla f(\vect{x}^{\ast})
%   =
%   \sum_{i\in\mathcal{A}(\vect{x}^{\ast})}\lambda_i^{\ast}\,\nabla c_i(\vect{x}^{\ast})
%   % =
%   % \sum_{i\in\mathcal{E}}\lambda_i^{\ast}\,\nabla c_i(\vect{x}^{\ast})
%   % +
%   % \sum_{i\in\mathcal{I}\cap\mathcal{A}(\vect{x}^{\ast})} \lambda_i^{\ast}\,\nabla c_i(\vect{x}^{\ast})
% \end{equation}
% with \(\lambda_i^{\ast}\ge 0\) for \(i\in\mathcal{I}\) and if we now dot \eqref{eq:KKT_objective_gradient} with any \(\vect{w}\in\mathcal{F}(\vect{x}^{\ast})\) we obtain
% \[
% \vect{w}^{\top}\nabla f(\vect{x}^{\ast})
% % =
% % \vect{w}^{\top}\sum_{i\in\mathcal{A}(\vect{x}^{\ast})}\lambda_i^{\ast}\,\nabla c_i(\vect{x}^{\ast})
% =\underbrace{
%    \sum_{i\in\mathcal{E}}
%      \lambda_i^{\ast}\,
%      \vect{w}^{\top}\nabla c_i(\vect{x}^{\ast})
%  }_{=0\;\text{because }\vect{w}^{\top}\nabla c_i=0}
%  +\;
%  \underbrace{
%    \sum_{i\in\mathcal{I}\cap\mathcal{A}(\vect{x}^{\ast})}
%      \lambda_i^{\ast}\,
%      \vect{w}^{\top}\nabla c_i(\vect{x}^{\ast})
%  }_{\ge 0\;\text{since }\lambda_i^{\ast}\ge 0,\
%                          \vect{w}^{\top}\nabla c_i\ge 0}
% \ge 0
% \]
...
% \sum_{i\in\mathcal{E}}
% \lambda_i^{\ast}
% \underbrace{\vect{w}^{\top} \nabla c_i(\vect{x}^{\ast})}_{= 0}
% +
% \sum_{i\in\mathcal{I}\cap\mathcal{A}(\vect{x}^{\ast})} 
% \lambda_i^{\ast}
% \underbrace{\vect{w}^{\top}\nabla c_i(\vect{x}^{\ast})}_{\ge 0}
% \ge 0
% \end{equation}
% and recall condition \eqref{eq:KKT_dual_feas}.
