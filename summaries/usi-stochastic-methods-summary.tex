...
A \emph{random number} is an unpredictable value generated independently of other numbers, lacking any discernible pattern.
...
\begin{definition}[Random Number Generator]
    ...
    \begin{itemize}[before={\parskip=0pt}, nosep]
        \item $S$ is the \emph{seed space} (a finite set of initial states). A seed \(s \in S\) is used to initialize the RNG.
        \item $T$ is the \emph{target space} (typically the interval $[0,1)$ or a set of integers).
...
A high-quality random number generator should have the following properties:
    \begin{itemize}[before={\parskip=0pt}, nosep]
        \item \emph{Unpredictability}: Future values cannot be deduced without knowing the seed and algorithm.
        \item \emph{Reproducibility}: Given the same seed, the RNG should produce the same sequence of numbers.
        \item \emph{Representation of True Randomness}: All outcomes have an equitable chance.
        \item \emph{Long period}: The sequence of numbers should be long before repeating.
        \item \emph{Efficiency}: The RNG should generate numbers quickly.
    \end{itemize}

\begin{example}
    The \emph{linear congruential generator} generates a sequence of random numbers via the following linear recurrence relation:
    \[
    ...
    \begin{itemize}[before={\parskip=0pt}, nosep]
        ...
        \item $a, c$, and $m$ are constants, known as the \emph{multiplier}, \emph{increment}, and \emph{modulus}, respectively.
...
\begin{example}
    The \emph{PCG64 RNG} is the default generator in newer versions of NumPy. 
    ...
    \begin{enumerate}
        \item \emph{State Update}: The 128-bit state $s_{n}$ is updated via the LCG:
        \[
        ...
        \end{itemize}
        \item \emph{Output Permutation}: A permutation is applied to the updated state to produce the final output.
    \end{enumerate}
...
\]
we can define a probability measure $\operatorname{P}_{U}$ on $U$ by \emph{pulling back} $\mu$:
\[
...
\begin{definition}[Probability Mass Function]{}
    Let $X$ be a discrete random variable with range $R(X)$. The \emph{probability mass function} (pmf) of $X$ is the function
    \[
...
For a continuous random variable $X$ with range $R(X) \subseteq \mathbb{R}$, the distribution is described by the \emph{probability density function} (pdf) $f_{X}$. This function is nonnegative and satisfies
\[
...
\begin{definition}{Cumulative Distribution Function}{}
    The \emph{cumulative distribution function} (CDF) of a random variable $X$ is defined by
    \[
...
\begin{tikzpicture}[scale=0.8]
  \begin{axis}[
    ...
    % y-values
    \pgfmathsetmacro{\yA}{exp(-abs(2*0.8))}
    \pgfmathsetmacro{\yR}{exp(-2^2*0.1)}

    %--- measure boxes (same positions as your labels), invisible ---
    \coordinate (A1) at (axis cs:2,\yA);
    \coordinate (A0) at (axis cs:2,0);
    \coordinate (R1) at (axis cs:2,\yR);

    % place nodes at the *same* positions the labels will be
    \path (A0) -- (A1)
      node[pos=0.5, right, anchor=west, inner sep=1pt, outer sep=0pt, xshift=0.1cm,
           text opacity=0, name=accbox]{\footnotesize Accept};
    \path (A1) -- (R1)
      node[pos=0.5, right, anchor=west, inner sep=1pt, outer sep=0pt, xshift=0.1cm,
           text opacity=0, name=rejbox]{\footnotesize Reject};

    %--- clip only the plots to (axis) MINUS the two text boxes (with tiny padding) ---
    \begin{scope}[even odd rule]
      \clip
        (rel axis cs:0,0) rectangle (rel axis cs:1,1)
        ([xshift=-1pt,yshift=-1pt]accbox.south west) rectangle
        ([xshift= 1pt,yshift= 1pt]accbox.north east)
        ([xshift=-1pt,yshift=-1pt]rejbox.south west) rectangle
        ([xshift= 1pt,yshift= 1pt]rejbox.north east);

      \addplot[black,dashed,thick,samples=101,clip=false,forget plot] {exp(-x^2*0.1)};
      \addplot[black,thick,samples=101,clip=false,forget plot] {exp(-abs(x*0.8))};
    \end{scope}
    ...
    % visible annotations (drawn after)
    \draw[red,dotted,very thick] (A1) -- node[pos=0.5, right, anchor=west, xshift=0.1cm, inner sep=1pt]
      {\textcolor{black}{\footnotesize Reject}} (R1);
    \draw[green,dotted,very thick] (A0) -- node[pos=0.5, right, anchor=west, xshift=0.1cm, inner sep=1pt]
      {\textcolor{black}{\footnotesize Accept}} (A1);

    % % DEBUG (optional): show exact boxes TikZ used
    % \draw[magenta,thin,dashed] (accbox.south west) rectangle (accbox.north east);
    % \draw[magenta,thin,dashed] (rejbox.south west) rectangle (rejbox.north east);
...
\begin{itemize}
  \item \emph{Degree Distribution:} In \(G(n,p)\), the degree \(k\) of any node follows a binomial distribution,
  \[
  ...
  \]
  \item \emph{Connectivity and Phase Transition:}
  ...
  \item \emph{Clustering Coefficient:} 
...
\begin{itemize}[leftmargin=1em]
  \item \emph{Martingale}: $\Exp[W_t\mid \mathcal F_s]=W_s$.
  \item \emph{Scaling}: $c^{-1/2}W_{ct}$ is again Brownian.
  \item \emph{Quadratic variation}: $[W]_t = t$.
  \item \emph{Nowhere differentiable}: paths are rough on every scale.
\end{itemize}
