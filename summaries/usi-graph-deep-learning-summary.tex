...
\Bigl(
\sum\nolimits_{j\in N(i)}
a_{ji} \vect{x}_j \matr{\Theta}
\Bigr)
\]
with $a_{ji}=\tilde{\matr{A}}_{ji}$.
So it fits into \autoref{def:message_passing} with 
\(
\mess(\vect{x}_i,\vect{x}_j,\vect{e}_{ji}) = a_{ji} \vect{x}_j \matr{\Theta}
...
\begin{equation}\label{eq:gat_scores}
    \alpha_{ji}=\sigma\bigl([\vect{x}_i' \mathbin{\|} \vect{x}_j']\vect{\theta}_2\bigr)
\end{equation}
\item 
