...
In 1984, Gilles Brassard and Charles Bennett developed the first application of quantum mechanics for cryptographic purposes with such a key agreement protocol (\hl{BB84}).
...
\begin{tikzpicture}[
  x=1cm, y=1cm,
  mirror/.style={line width=2.0pt, draw=black!90},
  bsplate/.style={line width=7pt, draw=black!20},
  bsedge/.style={line width=1.2pt, draw=black!70},
  ray/.style={line width=1.6pt, draw=black!90},
  arr/.style={-{Stealth[length=4mm,width=2.2mm]}, line width=1.1pt, draw=black!90}
]
  % dimensions
  \def\W{7.0}      % arm width
  \def\H{4.0}      % arm height
  \def\Lin{2.0}    % input extension
  \def\Lout{1.3}   % output extension
  \def\D{0.70}     % half-length of diagonal optics (45 deg)
  \def\Dash{1.75}  % dashed detector/input branch length
  \def\R{0.45}     % detector radius
  \def\SW{1.15}    % sample width
  \def\SH{0.90}    % sample height
  ...
  \coordinate (In)  at (-\Lin,0);
  \coordinate (Out) at (\W+\Lout,\H);
  \coordinate (Bin) at (0,-\Dash);
  \coordinate (Top) at (\W,\H+\Dash);
  \path ($ (M1)!0.5!(BS2) $) coordinate (S);

  % optical elements (all diagonals at 45 deg)
  \draw[bsplate] ($(BS1)+(-\D,-\D)$) -- ($(BS1)+(\D,\D)$);
  \draw[bsedge]  ($(BS1)+(-\D,-\D)$) -- ($(BS1)+(\D,\D)$)
    node[pos=1.0, anchor=north west, xshift=-1mm, yshift=-1mm] {Beam-splitter};
  \draw[bsplate] ($(BS2)+(-\D,-\D)$) -- ($(BS2)+(\D,\D)$);
  \draw[bsedge]  ($(BS2)+(-\D,-\D)$) -- ($(BS2)+(\D,\D)$)
    node[pos=1.0, anchor=south west, xshift=-1mm, yshift=-1mm] {Beam-splitter};
  \draw[mirror] ($(M1)+(-\D,-\D)$) -- ($(M1)+(\D,\D)$)
    node[pos=1.0, anchor=south west, xshift=-1mm] {Mirror};
  \draw[mirror] ($(M2)+(-\D,-\D)$) -- ($(M2)+(\D,\D)$)
    node[pos=1.0, anchor=north west, xshift=1mm] {Mirror};
  ...
  \draw[ray] (M1) -- (Out);
  \draw[ray] (M2) -- (BS2);
  \draw[dashed, line width=1.6pt, draw=black!90] (BS1) -- (Bin)
    node[pos=1.0, anchor=north] {$\hat{b}_{in}$};
  \draw[dashed, line width=1.6pt, draw=black!90] (BS2) -- (Top);

  % direction arrows
  \draw[arr] ($(In)!0.25!(BS1)$) -- ($(In)!0.45!(BS1)$);
  \draw[arr] ($(BS1)!0.40!(M2)$) -- ($(BS1)!0.48!(M2)$);
  \draw[arr] ($(BS1)!0.30!(M1)$) -- ($(BS1)!0.48!(M1)$);
  \draw[arr] ($(M2)!0.32!(BS2)$) -- ($(M2)!0.50!(BS2)$);
  \draw[arr] ($(M1)!0.37!(BS2)$) -- ($(M1)!0.46!(BS2)$);
  \draw[arr] ($(BS2)!0.28!(Out)$) -- ($(BS2)!0.72!(Out)$);

  % sample + phase
  \filldraw[fill=orange!70, draw=red!80!black, line width=1.2pt, opacity=0.9]
    ($(S)+(-0.5*\SW,-0.5*\SH)$) rectangle ($(S)+(0.5*\SW,0.5*\SH)$);
  \node[text=red!80!black, font=\small] at ($(S)+(0,0.95)$) {Sample};
  \node at ($(S)+(0,-0.80)$) {$\theta$};

  % mode label
  \node[anchor=east] at (In) {$\hat{a}_{in}$};

  % detectors: both are semicircles
  \draw[line width=1.2pt] ($(Out)+(0,\R)$) -- ($(Out)+(0,-\R)$)
    arc[start angle=-90, end angle=90, radius=\R]
    node[pos=1.0, anchor=west, rotate=-90, xshift=1mm] {Detector 1};
  \draw[line width=1.2pt] ($(Top)+(-\R,0)$) arc[start angle=180, end angle=0, radius=\R]
    node[pos=1.0, anchor=south east, yshift=2mm] {Detector 2};
\end{tikzpicture}
\]
