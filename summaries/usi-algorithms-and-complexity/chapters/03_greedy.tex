...
Moreover, in that case the new \nameref{def:mst} is \(T_1 = T_0 \cup \{e'\} \setminus \{f\}\).
\end{theorem}
...
Taking the contrapositive shows: if the MST changes, then $c_{e'}<c_f$.

\medskip
\underline{`$\Leftarrow$' ($c_{e'}<c_f$ implies change and gives $T_1$):}

Assume $c_{e'}<c_f$ and define
\[
T_1 := T_0 \cup \{e'\} \setminus \{f\}.
\]
