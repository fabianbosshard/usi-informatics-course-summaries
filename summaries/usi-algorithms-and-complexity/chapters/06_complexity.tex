...
\[
\pgfmathsetmacro{\nodesize}{0.35} % node diameter
\pgfmathsetmacro{\off}{0.15} % offset distance from each tree edge
\pgfmathsetmacro{\R}{\nodesize/2 + \off}   % rounding radius around each node
\begin{tikzpicture}[
  x=1.05cm,y=1.05cm,
  font=\tiny,
  v/.style={circle,draw,thick,minimum size=\nodesize cm,inner sep=0pt, font=\tiny},
  ...
  tour/.style={
    ...
    decoration={
      markings,
      mark=at position 0.08 with {\arrow{Triangle[length=1.2mm, width=0.8mm]}},
      mark=at position 0.26 with {\arrow{Triangle[length=1.2mm, width=0.8mm]}},
      mark=at position 0.38 with {\arrow{Triangle[length=1.2mm, width=0.8mm]}},
      mark=at position 0.51 with {\arrow{Triangle[length=1.2mm, width=0.8mm]}},
      mark=at position 0.615 with {\arrow{Triangle[length=1.2mm, width=0.8mm]}},
      mark=at position 0.77 with {\arrow{Triangle[length=1.2mm, width=0.8mm]}},
      mark=at position 0.88 with {\arrow{Triangle[length=1.2mm, width=0.8mm]}},
      mark=at position 1.0 with {\arrow{Triangle[length=1.2mm, width=0.8mm]}},
    }
  },
  font=\large
]
...
% ------------------------------------------------------------
% Turn macros
% Each turn defines the arc at the middle vertex and returns:
%   start angle a, delta angle d, and arc-start coordinate S
% ------------------------------------------------------------
...
\begin{scope}
  \placevertices
  \mstedges
  \node[anchor=east, font=\tiny] at ($(A)+(-0.2,0)$) {start};
  ...
  % A->C->D, C->D->C, D->C->E, C->E->C, E->C->A, C->A->B, A->B->F, B->F->G, F->G->F, G->F->H, F->H->F, H->F->B, F->B->A, B->A->C  (last arc omitted)
  ...
  % 12) at F: H->F->B  (+360 special)
  \TurnNormalPlus{H}{F}{B}{\aTwelve}{\dTwelve}{S12}{P12}

  % 13) at B: F->B->A  (+360 special)
  \TurnNormalPlus{F}{B}{A}{\aThirteen}{\dThirteen}{S13}{P13}

  % 14) at A: B->A->C  (compute, but we will OMIT drawing this arc)
  \TurnNormal{B}{A}{C}{\aFourteen}{\dFourteen}{S14}{P14}

  % ----------------------------------------------------------
  % Draw: always "-- (S_i) arc(...)".
  % Omit the very last arc (turn #14 at A) by stopping at S14.
  % ----------------------------------------------------------
  \draw[tour]
    (S0)
    ...
    -- (S14);
    % arc[start angle=\aFourteen, delta angle=\dFourteen, radius=\R]
    % -- cycle;
