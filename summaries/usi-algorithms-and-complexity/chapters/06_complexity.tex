...
\vspace{0.5em}
\[
\pgfmathsetmacro{\shiftamount}{7.5}      % panel spacing (TikZ units)
\pgfmathsetmacro{\nodesize}{0.2}     % node diameter (cm)
\pgfmathsetmacro{\off}{0.2}          % offset distance from each tree edge
\pgfmathsetmacro{\R}{\nodesize+\off}% rounding radius around each node
\tikzset{mytri/.tip={Triangle[length=1.0mm,width=0.8mm]}}
\begin{tikzpicture}[
  font=\footnotesize,
  x=0.45cm,y=0.45cm,
  v/.style={thick, circle,draw,minimum size=\nodesize cm,inner sep=0pt},
  e/.style={thick, line cap=round},
  % your triangle arrow
  % directed edges: SAME triangle arrow
  dir/.style={thick, -{mytri},line cap=round},
  % dotted tour + repeated triangle arrows along the path
  tour/.style={
    thick,
    densely dotted,
    postaction={decorate},
    decoration={
      markings,
      mark=at position 0.08  with {\arrow{mytri}},
      mark=at position 0.26  with {\arrow{mytri}},
      mark=at position 0.4  with {\arrow{mytri}},
      mark=at position 0.5  with {\arrow{mytri}},
      mark=at position 0.595 with {\arrow{mytri}},
      mark=at position 0.73  with {\arrow{mytri}},
      mark=at position 0.895  with {\arrow{mytri}},
      mark=at position 1.0   with {\arrow{mytri}},
    }
  },
  font=\large
]
...
% ------------------------------------------------------------
% Tree (nodes + edges): CV-extracted centers (affine-fixed)
% A=(0,0), B=(2,0), y up, |AB|=2
% ------------------------------------------------------------
\def\placevertices{%
  \node[v] (A) at ( 0.000,  0.000) {};
  \node[v] (B) at ( 2.000,  0.000) {};
  \node[v] (C) at ( 0.000, -1.496) {};
  \node[v] (D) at (-1.008, -2.992) {};
  \node[v] (E) at ( 0.992, -4.000) {};
  \node[v] (F) at ( 3.008, -1.496) {};
  \node[v] (G) at ( 2.000, -2.504) {};
  \node[v] (H) at ( 4.000, -2.992) {};
}
\def\mstedges{%
  \draw[e] (A)--(B);
  \draw[e] (A)--(C);
...
% ------------------------------------------------------------
% Geometry: right-offset intersection with circle radius R
% (used for the continuous twice-around tour)
% ------------------------------------------------------------
...
\newcommand{\ArcCCW}[3]{% V P Q -> \angStart, \deltaCCW in [0,360)
...
\newcommand{\ArcShort}[3]{% V P Q -> \angStart, \deltaShort in (-180,180]
  \AngleAt{#1}{#2}\xdef\angStart{\pgfmathresult}%
  \AngleAt{#1}{#3}\xdef\angEnd{\pgfmathresult}%
...
% Turn macros
\newcommand{\TurnNormal}[7]{% u v w  aMacro dMacro  Scoord  scratch
...
\newcommand{\TurnNormalPlus}[7]{% u v w  aMacro dMacro  Scoord  scratch
  \RightEndCoord{#1}{#2}{#7in}%
  \RightStartCoord{#2}{#3}{#7out}%
  \ArcCCW{#2}{#7in}{#7out}%
  \xdef#4{\angStart}%
...
\begin{scope}[shift={(0,0)}]
  \placevertices
  \mstedges
  \node[font=\footnotesize] at (1.5,-5) {MST};
\end{scope}

% ============================================================
% Panel 2: Twice-around tour (YOUR fixed continuous construction)
% ============================================================
\begin{scope}[shift={(\shiftamount,0)}]
  \placevertices
  \mstedges
  \node[anchor=east, font=\scriptsize] at ($(A)+(-0.2,0)$) {start};

  % Start at A and go to C first (A->C->D ...)
  \RightStartCoord{A}{C}{P0tmp}
  \AngleAt{A}{P0tmp}\xdef\aZero{\pgfmathresult}
  \SnapArcStart{A}{\aZero}{S0}

  % Tour (by turns, starting at A->C->D):
  \TurnNormal{A}{C}{D}{\aOne}{\dOne}{S1}{P1}
  \TurnLeaf{C}{D}{\aTwo}{\dTwo}{S2}{P2}
  \TurnNormal{D}{C}{E}{\aThree}{\dThree}{S3}{P3}
  \TurnLeaf{C}{E}{\aFour}{\dFour}{S4}{P4}
  \TurnNormalPlus{E}{C}{A}{\aFive}{\dFive}{S5}{P5}
  \TurnNormal{C}{A}{B}{\aSix}{\dSix}{S6}{P6}
  \TurnNormal{A}{B}{F}{\aSeven}{\dSeven}{S7}{P7}
  \TurnNormal{B}{F}{G}{\aEight}{\dEight}{S8}{P8}
  \TurnLeaf{F}{G}{\aNine}{\dNine}{S9}{P9}
  \TurnNormal{G}{F}{H}{\aTen}{\dTen}{S10}{P10}
  \TurnLeaf{F}{H}{\aEleven}{\dEleven}{S11}{P11}
  \TurnNormalPlus{H}{F}{B}{\aTwelve}{\dTwelve}{S12}{P12}
  \TurnNormalPlus{F}{B}{A}{\aThirteen}{\dThirteen}{S13}{P13}
  \TurnNormal{B}{A}{C}{\aFourteen}{\dFourteen}{S14}{P14}

  % draw (omit last arc, stop at S14)
  \draw[tour]
    (S0)
  ...
  \node[font=\footnotesize] at (1.5,-5) {Twice-around tour};
\end{scope}

% ============================================================
% Panel 3: Shortcut tour
% ============================================================
\begin{scope}[shift={({2*\shiftamount},0)}]
  \placevertices
  \node[anchor=east, font=\scriptsize] at ($(A)+(-0.2,0)$) {start};
  ...
  \node[font=\footnotesize] at (1.5,-5) {Shortcut tour};
\end{scope}

% ============================================================
% Panel 4: Optimum tour
% ============================================================
\begin{scope}[shift={({3*\shiftamount},0)}]
  \placevertices
  ...
  \node[font=\footnotesize] at (1.5,-5) {Optimum tour};
\end{scope}
