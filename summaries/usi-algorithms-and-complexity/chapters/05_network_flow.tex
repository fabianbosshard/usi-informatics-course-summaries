...
Given a flow $f$, each original edge $e=(u,v)\in E$ gives rise to up to two
\emph{residual arcs}:
\begin{itemize}
\item a \emph{forward residual arc} $e^+$ from $u$ to $v$ with residual capacity
\begin{equation}\label{eq:residual_capacity_forward}
c_f(e^+) \coloneqq c(e)-f(e)
\end{equation}
included if $f(e)<c(e)$
\item a \emph{backward residual arc} $e^-$ from $v$ to $u$ with residual capacity
\begin{equation}\label{eq:residual_capacity_backward}
c_f(e^-) \coloneqq f(e)
\end{equation}
included if $f(e)>0$
\end{itemize}

Each residual arc $a\in E_f$ arises from a unique original edge $e\in E$ (its \emph{parent edge}); denote this edge by $\pi(a)$.
We indicate whether $a$ is \texttt{forward} or \texttt{backward} by \(\sign(a)\).
We view a residual arc as a labeled object \(a=(e,\sigma)\) with \(\sigma\in\{\texttt{forward}, \texttt{backward}\}\), so its parent edge is \(\pi(a)=e\) and \(\sign(a)=\sigma\).

\begin{definition}[Residual Network]\label{def:residual_network}
The \emph{residual network} $G_f=(V,E_f)$ with respect to flow $f$ is the directed multigraph on vertex set $V$
whose arc multiset $E_f$ contains exactly all residual arcs $e^+$ and $e^-$ with positive
residual capacity, as defined in \eqref{eq:residual_capacity_forward}--\eqref{eq:residual_capacity_backward}.
\end{definition}

If both $(u,v)$ and $(v,u)$ are edges in $G$, then $G_f$ may contain two distinct residual arcs from $u$ to $v$; 
hence we view $G_f$ as a directed multigraph.

Since each edge of the original network \(G\) may result in the generation of at most two arcs in the residual network \(G_f\), 
the latter is of the same asymptotic size as \(G\).
% Also note that when \(v(f) = 0\), we have \(G_f = G\). % only true if there is no cycle
% Note that when \(f \equiv 0\), we have \(G_f = G\). % not really the same objects
Note that if $f\equiv 0$, then $G_f$ contains exactly the forward residual arc $e^+=(e,\texttt{forward})$
for every $e=(u,v)\in E$, with residual capacity $c(e)$; in particular, $G_f$ has the same
reachability structure as $G$.
...
\begin{equation}\label{eq:augmenting_path_capacity}
  {\color{Red}\boxed{\color{black}
  c_f(P) \coloneqq \min_{a \in P} c_f(a)
  }}
\end{equation}
...
%   \EndIf
% \EndFor
\ForAll{residual arc $a \in P$}
  \State $e \gets \pi(a)$ \Comment{retrieve original edge}
  \If{$\sign(a) = \texttt{forward}$} \Comment{$a$ is forward}
     \State $f(e) \gets f(e) + b$
  \Else \Comment{$a$ is backward}
     \State $f(e) \gets f(e) - b$
  \EndIf
\EndFor
\State \Return \(f\)
...
\EndFor
\State \(G_f \gets \text{\nameref{def:residual_network}}\) \Comment{initially only forward arcs with \(c_f(e^+) = c(e)\), since \(f \equiv 0\)}
...
\textcolor{AccentBlue}{Min cut} (at the end of \nameref{alg:ford_fulkerson}):
% \begin{itemize}
%   \item Let \(A\) be the set of vertices reachable from the source \(s\) in residual network \(G_f\).
%   \begin{itemize}
%     \item by definition of \(A\), \(s \in A\)
%     \item since \(G_f\) contains no augmenting path, \(t \notin A\)
%   \end{itemize}
%   \item \((A,B)\) with \(B = V \setminus A\) is a cut
%   \item \((A,B)\) is a \nameref{prob:min_cut} by \autoref{thm:max-flow-min-cut}.
% \end{itemize}
Let \(A\) be the set of vertices reachable from the source \(s\) in residual network \(G_f\).
By definition of \(A\), \(s \in A\).
Since \(G_f\) contains no augmenting path, \(t \notin A\).
\((A,B)\) with \(B = V \setminus A\) is a cut.
\((A,B)\) is a \nameref{prob:min_cut} by \autoref{thm:max-flow-min-cut}.
...
If $(u,v)\in E$ and $f(u,v)<c(u,v)$, 
then the forward residual arc $(u,v)$ has positive residual capacity and is therefore present in $G_f$, 
implying $v\in A$, a contradiction. 
Hence $f(u,v)=c(u,v)$.
Similarly, if $(v,u)\in E$ and $f(v,u)>0$, then the backward residual arc $(u,v)$ is present in $G_f$,
again implying $v\in A$, a contradiction. 
Hence $f(v,u)=0$.
...
\autoref{alg:ford_fulkerson} terminates in at most \(v(f^*)\) iterations, where \hl[7]{\(v(f^*) \leq nC\)} is the value of a maximum flow.
\end{theorem}
\begin{proof}
...
\begin{corollary}
\autoref{alg:ford_fulkerson} runs in time \hl[1]{\(O(v(f^*) \cdot m)\)} (not polynomial in \(n\)\textcolor{red}{!}), since an \hl[8]{\nameref{def:augmenting_path} can be found in \(O(m)\) time}.
\end{corollary}

Since \(v(f^*) \leq nC\), we also have that \autoref{alg:ford_fulkerson} runs in time \hl[2]{\(O(n C m)\)}.
...
\subsubsection{Edmonds-Karp (BFS)}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item with \nameref{alg:bfs}, we take the least number of edges
    \item with \nameref{alg:dfs}, we do not have that property\textcolor{red}{!}
    \item \nameref{alg:bfs}: \(O(m + n) = O(m)\), since \(m \geq n - 1\) 
  \end{itemize}
...
\hl[3]{So we went from \(O(n C)\) iterations to \(O(n m)\) iterations!}
