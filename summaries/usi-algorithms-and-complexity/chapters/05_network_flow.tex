...
% \item
% by flow conservation \eqref{eq:flow_conservation} at every node of \(G\), we have a feasible circulation (ignore the flows along edges incident to \(s\) and \(t\)).
% \qedhere
% \end{itemize}
% \end{proof}
...
becomes exactly $f_{\text{in}}(v)-f_{\text{out}}(v)=d(v)$, so $f$ is a feasible circulation in $G$.
\qedhere
\end{proof}
...
\begin{itemize}
  ...
  \end{itemize}
  \item compute a feasible circulation \(f_1\) on \(G'\)
