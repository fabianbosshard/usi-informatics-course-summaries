...
- keep a ``frontier'' of verices that have been discovered but not yet processed (a FIFO queue)\\
...
\indent - a bipartite graph is \(2\)-colorable \\
\indent - a \(2\)-colorable graph is bipartite \\
\indent \(\Rightarrow\) a bipartite graph is a \(2\)-colorable graph
\end{definition}

many graph problems become:\\
\indent - easier if the underlying graph is bipartite (matching)\\
\indent - tractable if the underlying graph is bipartite (independent set)
...
\begin{corollary}[of \autoref{lem:layers_bipartite}]\label{cor:layers_bipartite}
  ...
  \item \(G\) is bipartite iff no edge joins two nodes of the same \nameref{alg:bfs} layer
  \qedhere
  \end{itemize}
\end{corollary}
...
\begin{minipage}[b]{0.35\textwidth}
  \centering
\begin{tikzpicture}[every node/.style={circle,draw,minimum size=5mm,inner sep=0pt},>=stealth, scale=0.7]
  % layers
  \usetikzlibrary{shapes.symbols}
  \definecolor{blob}{RGB}{190,190,190}
  \tikzset{
  ...
  % Edges
  \draw[rounded corners=4pt] (s) 
    -- ($($(s)!0.42!(z)$)!0.15!90:(z)$)
    -- ($($(s)!0.58!(z)$)!0.15!270:(z)$)
    -- (z);
  \draw (s)  -- ++(-0.1,-0.8);
  \draw (s)  -- ++(-1,-0.7);
  \draw[rounded corners=4pt] (x) 
    -- ($($(x)!0.5!(z)$)!0.1!270:(z)$)
    -- ($($(x)!0.6!(z)$)!0.1!90:(z)$)
    -- (z);
  \draw[rounded corners=4pt] (y) 
    -- ($($(y)!0.4!(z)$)!0.1!90:(z)$)
    -- ($($(y)!0.5!(z)$)!0.1!270:(z)$)
    -- (z);
  \draw[red] (x) -- (y);
...
When during an \nameref{alg:dfs} an edge \((u,v)\) is first explored, the color of \(v\) yields information about the edge\tikzmark{edge-color}:
\begin{tikzpicture}[
  remember picture,
  overlay,
  scale = 0.5,
...
\coordinate (edge-color-y) at (pic cs:edge-color);
\coordinate (edge-color-border) at (current page.east |- edge-color-y);
\coordinate (edge-color-margin) at ($(edge-color-border) + (\textwidth,0) + (-\paperwidth,0)$);
% \node[circle, draw=red, minimum size=2mm, inner sep=0pt] at (edge-color-margin) {}; % debugging
\begin{scope}[shift={(edge-color-margin)}, scale=0.8]
...
\begin{theorem}
\autoref{alg:topological_sort_recursive} finds a TO in \(O(n + m)\) time.
\end{theorem}
\begin{proof}
...
\begin{aligned}
& (x,w)\ \text{is a back edge for some}\\[-4pt]
...
\begin{scope} % left figure
  ...
  % dotted back edges
  \draw[thick, densely dotted] (-0.3,1.5) to[bend left=40] (u);
  \draw[thick, densely dotted] (-0.7,0.25) to[bend left=55] (-0.35,1.3);
  \draw[thick, densely dotted] (0.7, 0.25) to[bend right=55] (v);
  \draw[thick, densely dotted] (-0.5,1.0) to[bend left=45] (w);
...
\begin{scope}[xshift=5cm] % mid figure (cut vertex)
  ...
  \draw[thick, densely dotted] (-0.3,1.5) to[bend left=45] (u);
  \draw[thick, densely dotted] (-0.7,0.25) to[bend left=55] (-0.35,1.3);
  \draw[thick, densely dotted] (0.7, 0.25) to[bend right=55] (v);
...
\begin{scope}[xshift=10cm] % right figure (bridge)
  ...
  % dotted back edges / paths 
  \draw[thick, densely dotted] (-0.5,1.0) to[bend left=45] (v);
  \draw[thick, densely dotted] (-0.7,0.25) to[bend left=55] (-0.35,1.3);
  \draw[thick, densely dotted] (0.7, 0.25) to[bend right=55] (v);
...
- \hyperref[thm:strongly_connected_test]{recall} that we can answer if a digraph is strongly connected by 2 \nameref{alg:bfs}/\nameref{alg:dfs} 
