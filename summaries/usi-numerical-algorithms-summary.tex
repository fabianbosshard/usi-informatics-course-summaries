...
\definecolor{emblue}{RGB}{20,60,120}
% \renewcommand{\emph}[1]{\textcolor{black}{\textsl{#1}}}
\renewcommand{\emph}[1]{\textcolor{black}{\textit{#1}}}
...
\begin{algorithm}[h]
  \caption{de Casteljau}% (evaluate Bézier curve at \(t\))}
...
\begin{equation}\label{eq:bernstein_polynomials}
{\color{red}\boxed{\color{black}
B_i^{n}(s)=\binom{n}{i}s^i(1-s)^{n-i}
}}
\end{equation}
where \(i=0,\dots,n\) and \[\binom{n}{i}=\frac{n!}{i!(n-i)!}\] is the binomial coefficient.
\[
\begin{tikzpicture}
\begin{axis}[
  width=7cm,
...
\newcommand{\propitem}[2]{%
  \item \makebox[\propsep][l]{\textcolor{red}{#1}}%
  \parbox[t]{\dimexpr\linewidth-\propsep\relax}{\textcolor{black}{#2}}%
}
\settowidth{\propw}{{Endpoint interpolation:}} % longest label in this list
\setlength{\propsep}{\propw + 0.5em} % label + space

\begin{fact}\label{fact:bernstein-bezier-properties}
Besides forming a basis for the space of polynomials of degree at most \(n\), the Bernstein polynomials satisfy the following properties:
\begin{enumerate}[label=•]%[label=\textbf{\roman*.}]
  ...
  \propitem{{Recursion formula:}}{%
    $B_i^n(s)=sB_{i-1}^{n-1}(s)+(1-s)B_i^{n-1}(s)$ with $B_i^n =0$ for $i<0$ or $i>n$}
  \propitem{{Derivative:}}{%
...
\begin{enumerate}[label=•]%[label=\textbf{\roman*.}]
  ...
  \propitem{{Endpoint interpolation:}}{%
    $\vect{F}(a)=\vect{C}_0$ and $\vect{F}(b)=\vect{C}_n$\label{item:endpoint-interpolation}}
  \propitem{{Derivative:}}{%
    $ \vect{F}'(t)=\frac{n}{b-a}\sum_{i=0}^{n-1}(\vect{C}_{i+1}-\vect{C}_i)B_i^{n-1}(s)$}
  \propitem{{Endpoint tangent:}}{%
    $ \vect{F}'(a)=\frac{n}{b-a}(\vect{C}_1-\vect{C}_0)$ and $\vect{F}'(b)=\frac{n}{b-a}(\vect{C}_n-\vect{C}_{n-1})$\label{item:endpoint-tangent}}
  \propitem{{Symmetry:}}{%
...
In \(\R^2\), Bézier curves satisfy the \emph{variation diminishing property}: 
any line intersects the curve \(\vect{F}\) no more often than it intersects its control polygon. 
Therefore,
\(\vect{F}\) cannot exhibit unexpected oscillations.
\end{fact}


% % numbered "equation" at end of the same line (works in inline math)
% \newcommand{\inlabeleq}[1]{%
%   \refstepcounter{equation}\label{#1}%
%   \quad\hfill(\theequation)%
% }
...
%   \propitem{{Partition of unity:}}{%
%     $\sum_{i=0}^{n} B_i^n(s)\equiv 1\inlabeleq{eq:bernstein-partition}$}
%   \propitem{{Positivity:}}{%
%     $B_i^n(s)\ge 0 \text{ for } s\in[0,1]\inlabeleq{eq:bernstein-positivity}$}
%   \propitem{{Endpoints:}}{%
%     $B_i^n(0)=\delta_{i,0}\ \text{and}\ B_i^n(1)=\delta_{i,n}\inlabeleq{eq:bernstein-endpoints}$
%     (where $\delta_{i,j}$ is the Kronecker delta)}
%   \propitem{{Symmetry:}}{%
%     $B_i^n(s)=B_{n-i}^n(1-s)\inlabeleq{eq:bernstein-symmetry}$}
%   \propitem{{Recursion formula:}}{%
%     $B_i^n(s)=sB_{i-1}^{n-1}(s)+(1-s)B_i^{n-1}(s)\inlabeleq{eq:bernstein-recursion}$
%     with $B_i^n =0$ for $i<0$ or $i>n$}
%   \propitem{{Derivative:}}{%
%     $\bigl(B_i^n\bigr)'(s)=n\bigl(B_{i-1}^{n-1}(s)-B_i^{n-1}(s)\bigr)\inlabeleq{eq:bernstein-derivative}$}
%   \propitem{{Locality:}}{%
%     $B_i^n \text{ has a local maximum at } s=i/n\inlabeleq{eq:bernstein-locality}$}
...
%   \propitem{{Affine invariance:}}{%
%     $\phi\!\left(\sum_{i=0}^n \vect{C}_iB_i^n(s)\right)
%       =\sum_{i=0}^n \phi(\vect{C}_i)B_i^n(s)\inlabeleq{eq:bezier-affine}$
%     for any affine mapping $\phi:\R^d\to\R^d$}
%   \propitem{{Convex hull:}}{%
%     $\vect{F}(t)\in\operatorname{conv}\{\vect{C}_0,\dots,\vect{C}_n\}\inlabeleq{eq:bezier-convexhull}$}
%   \propitem{{Endpoint interpolation:}}{%
%     $\vect{F}(a)=\vect{C}_0\ \text{and}\ \vect{F}(b)=\vect{C}_n\inlabeleq{eq:bezier-endpoints}$}
%   \propitem{{Derivative:}}{%
%     $\vect{F}'(t)=\frac{n}{b-a}\sum_{i=0}^{n-1}(\vect{C}_{i+1}-\vect{C}_i)B_i^{n-1}(s)\inlabeleq{eq:bezier-derivative}$}
%   \propitem{{Endpoint tangent:}}{%
%     $\vect{F}'(a)=\frac{n}{b-a}(\vect{C}_1-\vect{C}_0)
%       \inlabeleq{eq:bezier-endpoint-tangent-a}$
%     and
%     $\vect{F}'(b)=\frac{n}{b-a}(\vect{C}_n-\vect{C}_{n-1})
%       \inlabeleq{eq:bezier-endpoint-tangent-b}$}
%   \propitem{{Symmetry:}}{%
%     $\sum_{i=0}^n \vect{C}_iB_i^n(s)=\sum_{i=0}^n \vect{C}_{n-i}B_i^n(1-s)\inlabeleq{eq:bezier-symmetry}$}
%   \propitem{{Local control:}}{%
%     $\vect{C}_i \text{ influences }\vect{F}\text{ the most at } t=a+i(b-a)/n\inlabeleq{eq:bezier-localcontrol}$}
...
Another property is
\[\begin{aligned}
\sum_{i=0}^n \frac{i}{n} B_i^n(s)
&=\sum_{i=0}^n \frac{i}{n} \frac{n!}{i!(n-i)!} s^i (1-s)^{n-i} \\
...
Given a B\'ezier curve
\begin{equation}\label{eq:degree-elevation-start}
\vect{F}(t)=\sum_{i=0}^{n}\vect{C}_i\,B_i^{n}(s)
% \qquad\qquad
% s(t)=\frac{t-a}{b-a}\in[0,1]  % reduntant info
\end{equation}
of degree \(n\) with control points \(\vect{C}_0,\dots,\vect{C}_n\in\R^d\).
Then the same curve admits a representation of degree \(n+1\),
\begin{equation}\label{eq:degree-elevation-end}
\vect{F}(t)=\sum_{i=0}^{n+1}\vect{C}_i^{\ast}\,B_i^{n+1}(s)
\end{equation}
with control points
\(
...
\begin{proof}
Using \eqref{eq:bernstein_polynomials} and the binomial identities
\[
...
\]
Insert into \eqref{eq:degree-elevation-start}:
\[
...
High-degree Bézier curves are rarely used for designing complex curves for 2 reasons: \(\mathcal{O}(n^2)\) evaluation with \autoref{alg:de-casteljau} becomes slower and local control is not very effective for large \(n\), so the shape becomes difficult to control.
...
They are called \hl[2]{\(C^k\)-continuous at \(t_1\)} if
\(\vect{F}_0(t_1)=\vect{F}_1(t_1)\),
\(\vect{F}_0'(t_1)=\vect{F}_1'(t_1)\),
\(\dots\),
\(\vect{F}_0^{(k)}(t_1)=\vect{F}_1^{(k)}(t_1)\).


There is also the weaker notion of \emph{geometric continuity}.
The two curves \(\vect{F}_0\) and \(\vect{F}_1\) are called \hl[2]{\(G^1\)-continuous at \(t_1\)} if
\(
\vect{F}_0(t_1)=\vect{F}_1(t_1)
\) and \(
\vect{F}_0'(t_1)\parallel\vect{F}_1'(t_1)
\),
i.e. the tangent directions agree, but their magnitudes may differ.
Clearly, \(C^1\)-continuity implies \(G^1\)-continuity, but not conversely.
% Unlike \(C^1\)-continuity, \(G^1\)-continuity is invariant under reparameterization and is often sufficient for visual smoothness.


\newcommand{\conditem}[2]{%
  \item \makebox[\propsep][l]{\textcolor{black}{{#1}}}%
  \parbox[t]{\dimexpr\linewidth-\propsep\relax}{\textcolor{black}{#2}}%
}
\settowidth{\propw}{\(C^{-1}\)} % longest label in this list
\setlength{\propsep}{\propw + 1em} % label + space


\begin{example}[Curve design]
In graphic editors, curves are often represented as piecewise cubic Bézier curves.
At a join point \(P\),
\begin{itemize}
  \conditem{\(C^{-1}\):}{the segments do not share the same point \(P\) (gap)}
  \conditem{\(C^{0}\):}{the segments share the same point \(P\) (corner allowed)}
  \conditem{\(G^{1}\):}{the two Bézier handles at \(P\) are collinear (smooth join)}
  \conditem{\(C^{1}\):}{the handles are collinear and of equal length (matched derivatives)}
\end{itemize}
Thus, \(G^{1}\)-continuity corresponds to a visually smooth join, while \(C^{1}\)-continuity additionally enforces equal parametrization speed.
\end{example}

\subsubsection{\(C^0\)-continuity}
By the \hyperref[item:endpoint-interpolation]{endpoint interpolation property} from \autoref{fact:bernstein-bezier-properties} we have 
% \(\vect{F}_0(t_1)=\vect{C}_{0,n}\) and \(\vect{F}_1(t_1)=\vect{C}_{1,0}\), 
\[\begin{aligned}
\vect{F}_0(t_1) &= \vect{C}_{0,n} \\
\vect{F}_1(t_1) &= \vect{C}_{1,0}
\end{aligned}\]
so the 2 curves join continuously at \(t_1\) iff
\(
\vect{C}_{0,n}=\vect{C}_{1,0}
\).


\subsubsection{\(C^1\)-continuity}\label{sec:c1-continuity}
Similarly, by the \hyperref[item:endpoint-tangent]{endpoint tangent property} from \autoref{fact:bernstein-bezier-properties}, 
we have
% \(\vect{F}_0'(t_1)=\frac{n}{t_1-t_0}(\vect{C}_{0,n}-\vect{C}_{0,n-1})\) and \(\vect{F}_1'(t_1)=\frac{n}{t_2-t_1}(\vect{C}_{1,1}-\vect{C}_{1,0})\),
\[\begin{aligned}
\vect{F}_0'(t_1) &= \frac{n}{t_1-t_0}(\vect{C}_{0,n}-\vect{C}_{0,n-1}) \\
\vect{F}_1'(t_1) &= \frac{n}{t_2-t_1}(\vect{C}_{1,1}-\vect{C}_{1,0})
\end{aligned}\]
so
the 2 curves join with \(C^1\)-continuity at \(t_1\) iff
\(\vect{C}_{0,n-1}\), \(\vect{C}_{0,n} = \vect{C}_{1,0}\), \(\vect{C}_{1,1}\)  lie on a common line and are an affine image of the parameter values \(t_0\), \(t_1\), \(t_2\), 
i.e.,
\[
\vect{C}_{0,n} 
=
...
These 2 observations can be used to construct the simplest kind of Bézier splines, see \autoref{ex:cubic-bezier-spline}.




\settowidth{\propw}{\textbf{Natural end:}} % longest label in this list
\setlength{\propsep}{\propw + 1em} % label + space


\begin{example}[Cubic \(C^1\)-continuous Bézier splines]\label{ex:cubic-bezier-spline}
...
This is called a \emph{cubic Hermite spline}.



\paragraph{Automatic tangent choices.}
If tangents are not given, they can be generated from the interpolation points. For the interior tangents \(i=1,\dots,m-1\), 2 common choices are:
\begin{itemize}
  \conditem{\textbf{\tikzmarknode{mill}{FMILL}:}%
\begin{tikzpicture}[overlay,remember picture, font=\tiny,%
  >={Stealth[length=1.2mm]},
  ann/.style={Stealth-,rounded corners=2pt,line cap=round, thick}
]
  % ---- padding (numerator): go UP then RIGHT
  \coordinate (millA) at ($(mill.north) + (0,1.8mm)$);    % vertical end
  \coordinate (millB) at ($(millA) + (3mm,0)$);        % right end (tip)
  \draw[ann, ->, green!50!black] ($(mill.north) + (0,0.4mm)$) -- (millA) -- (millB);
  \node[anchor=west,inner sep=1pt, color=green!50!black] at (millB) {\textnormal{milling devices}};
\end{tikzpicture}%
}{\(
  \vect{T}_i=\frac{\vect{P}_{i+1}-\vect{P}_{i-1}}{t_{i+1}-t_{i-1}}
  \) (finite difference)}
  \conditem{\textbf{Bessel:}}{\(\vect{T}_i=\vect{Q}_i'(t_i)\), \(\vect{Q}_i\) is the quadratic interpolant to \(\vect{P}_{i-1}\), \(\vect{P}_i\), \(\vect{P}_{i+1}\) at \(t_{i-1}\), \(t_i\), \(t_{i+1}\)}
  % \((t_{i-1},\vect{P}_{i-1})\), \((t_i,\vect{P}_i)\), \((t_{i+1},\vect{P}_{i+1})\)
\end{itemize}

Endpoint tangents can be chosen via:
\begin{itemize}
  \conditem{\textbf{Overhauser:}}{\(\vect{T}_0=\vect{Q}_1'(t_0)\), \(\vect{T}_m=\vect{Q}_{m-1}'(t_m)\)}
  \conditem{\textbf{Natural end:}}{\(\vect{F}_0''(t_0)=\vect{F}_{m-1}''(t_m)=\vect{0}\) \(\Rightarrow\)
  \(
  ...
  \)}
\end{itemize}
For closed curves (\(\vect{P}_0=\vect{P}_m\)), endpoints are treated like interior points with the two choices above.
\end{example}

\subsubsection{\(C^2\) continuity}

The two curves \(\vect{F}_0\) and \(\vect{F}_1\) join with \(C^2\)-continuity at \(t_1\), if, in addition to the conditions from \autoref{sec:c1-continuity}, there exists a point \(\vect{D}\in\R^d\) such that
% \(
% \vect{C}_{0,n-1} 
% =
% (1-\lambda)\vect{C}_{0,n-2}+\lambda\vect{D}
% \)
% and
% \(
% \vect{C}_{1,1}
% =
% (1-\lambda)\vect{D}+\lambda\vect{C}_{1,2}
% \)
\[\begin{aligned}
\vect{C}_{0,n-1} 
&=
...
This is called the \emph{A-frame condition} and guarantees that 
\(\vect{C}_{0,n-2}\), \(\vect{C}_{0,n-1}\), \(\vect{D}\),
as well as
\(\vect{D}\), \(\vect{C}_{1,1}\), \(\vect{C}_{1,2}\)
lie on a common line and are affine images of the parameter values \(t_0\), \(t_1\), \(t_2\).

\begin{example}[Cubic \(C^2\)-continuous Bézier splines]\label{ex:cubic-c2-bezier-spline}
Let \(\vect{P}_0,\dots,\vect{P}_m\in\R^d\) be given at parameters
\(
t_0<\cdots<t_m
\).
Define cubic Bézier pieces \(\vect{F}_i:[t_i,t_{i+1}]\to\R^d\) with
\(
\vect{C}_{i,0}=\vect{P}_i
\),
\(
\vect{C}_{i,3}=\vect{P}_{i+1}
\).
Choose the two interior control points \(\vect{C}_{0,1}\) and \(\vect{C}_{0,2}\) (or symmetrically \(\vect{C}_{m-1,1}\) and \(\vect{C}_{m-1,2}\))
arbitrarily.
Then imposing \(C^1\) continuity and the A-frame condition at all interior knots
uniquely determines all remaining control points.
Hence a cubic \(C^2\)-continuous Bézier spline interpolating the data
...
The difficulty of controlling the shape of a \nameref{ex:cubic-c2-bezier-spline} motivates B-splines, which
are piecewise polynomial curves with built-in maximal smoothness and intuitive local control.

\subsubsection{Definition and evaluation}\label{subsec:bspline-definition}

A B-spline curve \(\vect{F}\) of degree \(n\) with \(m+1\) pieces is defined by
control points \(\vect{D}_0,\dots,\vect{D}_{n+m}\in\R^d\)
and knots 
% \(t_0,\dots,t_{2n+m+1}\) with
\[
\underbrace{
t_0
\le
...
\le
t_{n+m+1}
}_{\text{\(m+2\) knots}}%, domain of \(\vect{F}\)}}
\le
\underbrace{
t_{n+m+2}
\le
...
\]
i.e. a non-decreasing \emph{knot vector} of length \(2n+m+2\).
The curve domain is between the \(m+2\) ``internal'' knots, i.e. 
\(
\vect{F}:[t_n,t_{n+m+1}] \to \R^d
\).
To evaluate \(\vect{F}(t)\) for \(t\in[t_n,t_{n+m+1}]\),
first
find \(k\) such that
\(
t_{n+k}\le t\le t_{n+k+1}
\).
Then repeatedly compute convex combinations:
\[
\vect{D}_i^{0}(t)=\vect{D}_i
\]
where
\(i=k,\dots,k+n\),
and for \(j=1,\dots,n\),
...
\]
where \(i=k,\dots,k+n-j\) and then finally set
\(
\vect{F}(t)=\vect{D}_k^{\,n}(t)
\).
This is the \nameref{alg:de-boor} algorithm;
it is the B-spline analogue of \nameref{alg:neville} and \nameref{alg:de-casteljau}.


Like \nameref{alg:neville}'s algorithm and \nameref{alg:de-casteljau}'s algorithm, \nameref{alg:de-boor}'s algorithm costs \hl[6]{\(\mathcal{O}(n^2)\)}, but since here \(n\) is typically small (often \(2\le n\le 5\)), this is not an issue.



\begin{algorithm}[h]
  \caption{de Boor}% (evaluate B-spline curve at \(t\))}
  \label{alg:de-boor}
  \begin{algorithmic}[1]
    \Require control points \(\vect{D}_0,\dots,\vect{D}_{n+m}\in\R^d\), knots \(t_0,\dots,t_{2n+m+1}\), parameter \(t\in[t_n,t_{n+m+1}]\)
    \Ensure value \(\vect{F}(t)\)
    \State \(k\gets 0\)
    \While{\(t_{n+k+1}<t\)}
      \State \(k\gets k+1\)
    \EndWhile
    \For{\(i=0 \TO n\)}
      \State \(\vect{E}_i\gets \vect{D}_{i+k}\)
    \EndFor
    \For{\(j=1 \TO n\)}
      \For{\(i=0 \TO n-j\)}
        \State \(\vect{E}_i \gets \frac{t_{n+i+k+1}-t}{t_{n+i+k+1}-t_{i+k+j}} \vect{E}_i + \frac{t-t_{i+k+j}}{t_{n+i+k+1}-t_{i+k+j}} \vect{E}_{i+1}\)
      \EndFor
    \EndFor
...
The curve \(\vect{F}\) is polynomial of degree \(n\) on each interval \([t_{n+k},t_{n+k+1}]\)
and \(C^{n-1}\) at interior knots if knots are simple (i.e. not repeated).


\subsubsection{B-spline basis functions}\label{subsec:bspline-basis}

A B-spline curve can be written explicitly as
\begin{equation}\label{eq:bspline-curve}
\vect{F}(t)=\sum_{i=0}^{n+m}\vect{D}_i\,N_i^{n}(t)
\end{equation}
where \(t \in[t_n,t_{n+m+1}]\).
\(N_i^{n}\), \(i = 0,\dots,n+m\), are the \emph{B-spline basis functions} of degree \(n\) with respect to the knots \(t_0,\dots,t_{2n+m+1}\).

They are defined recursively as follows.
Define the piecewise constant functions
\[
N_i^{0}(t)
=
\chi_{[t_i,t_{i+1})}(t)
...
N_i^{j}(t)
=
\frac{t-t_i}{t_{i+j}-t_i}\,N_i^{j-1}(t)
+
...
\begin{fact}\label{fact:bspline-basis-properties}
The functions \(N_i^{j}\) are piecewise polynomials of degree \(j\) and \(C^{j-1}\)-continuous. They satisfy
\begin{enumerate}[label=•]
  \propitem{Partition of unity:}{%
    \(\sum_{i=0}^{2n+m-j} N_i^{j}(t)=1\) for \(t\in[t_j,t_{2n-j+m+1}]\)}
  \propitem{Positivity:}{%
    \(N_i^{j}(t)>0\) for \(t\in(t_i,t_{i+j+1})\)}
  \propitem{Local support:}{%
    \(N_i^{j}(t)=0\) for \(t\notin[t_i,t_{i+j+1}]\)}
  \propitem{Derivative:}{%
    \(\bigl(N_i^{j}\bigr)'(t)=\frac{j}{t_{i+j}-t_i}N_i^{j-1}(t)-\frac{j}{t_{i+j+1}-t_{i+1}}N_{i+1}^{j-1}(t)\)}
\end{enumerate}


These translate to B-spline curve properties.
For \(t\in[t_{n+k},t_{n+k+1}]\)
\begin{enumerate}[label=•]
  \propitem{Convex hull:}{%
    \(\vect{F}(t)\in\operatorname{conv}\{\vect{D}_k,\dots,\vect{D}_{k+n}\}\)}
  \propitem{Local control:}{%
    \(\vect{F}(t)\) depends only on \(\vect{D}_k,\dots,\vect{D}_{k+n}\)}
  \propitem{Derivative:}{%
    \(\vect{F}'(t)=\sum_{i=1}^{n+m}\vect{D}_i'\,N_i^{n-1}(t)\) with \(\vect{D}_i'=\frac{n}{t_{i+n}-t_i}\bigl(\vect{D}_i-\vect{D}_{i-1}\bigr)\)}
  \propitem{Affine invariance:}{%
    \(\phi\!\left(\sum_{i=0}^{n+m}\vect{D}_iN_i^{n}(t)\right)=\sum_{i=0}^{n+m}\phi(\vect{D}_i)N_i^{n}(t)\) for any affine \(\phi:\R^d\to\R^d\)}
\end{enumerate}
As for Bézier curves, B-spline curves also satisfy the variation diminishing property in \(\R^2\).
\end{fact}


\subsubsection{Interpolation}\label{subsec:cubic-bspline-interpolation}

Given points \(\vect{P}_0,\dots,\vect{P}_{m+1}\in\R^d\),
a standard approach is to interpolate them at the knots \(t_n,\dots,t_{n+m+1}\),
that is
\[
\vect{F}(t_{n+i})=\vect{P}_i
\]
for \(i=0,\dots,m+1\).
Using \eqref{eq:bspline-curve} this yields the linear system
\[
\matr{M}\vect{D}=\vect{P}
\]
with unknown control points \(
\vect{D}=[\vect{D}_0, \dots, \vect{D}_{n+m}]^\top %\in\R^{(n+m+1)\times d}
\), interpolation points \(
\vect{P}=[\vect{P}_0, \dots, \vect{P}_{m+1}]^\top %\in\R^{(m+2)\times d}
\),
and
\[
% \vect{D}=
...
\matr{M}=
\begin{bmatrix}
N_0^{n}(t_n) & N_1^{n}(t_n) & \cdots & N_{n+m}^{n}(t_n)\\
...
\in \R^{(m+2) \times (n+m+1)}
\]
This system is underdetermined with \(n-1\) degrees of freedom.
Hence, additional conditions are needed to get a unique solution.
Moreover, due to local support, \(\matr{M}\) is a band matrix with non-zero entries only near the diagonal.

\begin{example}[Cubic B-spline interpolation]
For cubic interpolation, \(n=3\), the matrix is tridiagonal up to two additional boundary rows.
The two free parameters correspond to the two degrees of freedom in \autoref{ex:cubic-c2-bezier-spline}.
But they can be fixed more conveniently via end conditions! \Smiley

\renewcommand{\conditem}[2]{%
  \item \makebox[\propsep][l]{\textcolor{black}{\textbf{#1}}}%
  \parbox[t]{\dimexpr\linewidth-\propsep\relax}{\textcolor{black}{#2}}%
}
\settowidth{\propw}{\textbf{Not-a-knot:}} % longest label in this list
\setlength{\propsep}{\propw + 2.5em} % label + space

Common choices are
\begin{itemize}
  \conditem{{Natural:}}{%
    \(\vect{F}''(t_3)=\vect{F}''(t_{m+4})=\vect{0}\)}
  \conditem{{Clamped:}}{%
    \(\vect{F}'(t_3)=\vect{T}_0\) \ and \  \(\vect{F}'(t_{m+4})=\vect{T}_{m+1}\)}
  \conditem{{Periodic:}}{%
    \(\vect{F}'(t_3)=\vect{F}'(t_{m+4})\) \ and \  \(\vect{F}''(t_3)=\vect{F}''(t_{m+4})\)}
  \conditem{{Not-a-knot:}}{%
    \(\lim\limits_{t\nearrow t_4}\vect{F}'''(t)=\lim\limits_{t\searrow t_4}\vect{F}'''(t)\) \ and \  \(\lim\limits_{t\nearrow t_{m+3}}\vect{F}'''(t)=\lim\limits_{t\searrow t_{m+3}}\vect{F}'''(t)\)}
\end{itemize}
These conditions can be expressed in terms of basis functions and control points and added as the first and last row,
turning the system into a square invertible system that uniquely determines the control points.

% \begin{remark}
In the functional setting, \(d=1\),
the natural interpolating cubic B-spline minimizes the bending energy
\[
\lVert f''\rVert_2^2=\int_{t_3}^{t_{m+4}}\bigl(f''(t)\bigr)^2\,\dif t
\]
