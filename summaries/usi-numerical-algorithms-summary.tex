...
\renewcommand{\emph}[1]{\textcolor{emblue}{\textsl{#1}}}
...
\begin{itemize}
  \item \textbf{\tikzmarknode{mill}{FMILL}:}%
\begin{tikzpicture}[overlay,remember picture, font=\tiny,%
  ...
  \node[anchor=west,inner sep=1pt, color=green!50!black] at (millB) {milling devices};
\end{tikzpicture} %
  \(
  \vect{T}_i=\frac{\vect{P}_{i+1}-\vect{P}_{i-1}}{t_{i+1}-t_{i-1}}
  \) (finite difference)
  \item \textbf{Bessel:} \(\vect{T}_i=\vect{Q}_i'(t_i)\), where \(\vect{Q}_i\) is the quadratic interpolant to \(\vect{P}_{i-1}\), \(\vect{P}_i\), \(\vect{P}_{i+1}\) at \(t_{i-1}\), \(t_i\), \(t_{i+1}\)
...
\begin{itemize}
  ...
  \item \textbf{Natural end:} \(\vect{F}_0''(t_0)=\vect{F}_{m-1}''(t_m)=\vect{0}\) \(\Rightarrow\)
  \(
...
Common choices are
\begin{itemize}
  \item \textbf{Natural:} \(\vect{F}''(t_3)=\vect{F}''(t_{m+4})=\vect{0}\)
  \item \textbf{Clamped:} \(\vect{F}'(t_3)=\vect{T}_0\) and \(\vect{F}'(t_{m+4})=\vect{T}_{m+1}\)
  \item \textbf{Periodic:} \(\vect{F}'(t_3)=\vect{F}'(t_{m+4})\) and \(\vect{F}''(t_3)=\vect{F}''(t_{m+4})\)
  \item \textbf{Not-a-knot:} \(\lim_{t\nearrow t_4}\vect{F}'''(t)=\lim_{t\searrow t_4}\vect{F}'''(t)\) and \(\lim_{t\nearrow t_{m+3}}\vect{F}'''(t)=\lim_{t\searrow t_{m+3}}\vect{F}'''(t)\)
\end{itemize}
