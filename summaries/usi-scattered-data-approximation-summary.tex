...
Often it is desirable that an approximation can represent polynomials exactly.
As we have seen in \autoref{theorem:mairhuber_curtis}, in this case the interpolation points need to be carefully chosen.
...
A function \(K: \R^d \to \R\) is called conditionally positive definite of order \((q+1)\), iff for any mutually distinct points \(\vect{x}_1, \ldots, \vect{x}_N \in \R^d\) and any \(N \in \N\) the generalized Vandermonde matrix \(\matr{A} = [K(\vect{x}_i - \vect{x}_j)]_{i,j=1}^N\) satisfies
...
Since \(K\) is strictly conditionally positive definite of order \((q+1)\), this implies \(\vect{c} = \vect{0}_N\).
...
\]
By the \(q\)-unisolvency of \(\vect{x}_1, \ldots, \vect{x}_N\), the matrix \(\matr{P}\) has full column rank \(m_q\).
...
In this way one passes from a normed space \(X\) to its completion \(\overline{X}\).
...
For a Hamel basis of a vector space, every vector can be represented as a \emph{finite} linear combination of basis vectors.
...
The idea of a Schauder basis is to drop the requirement that the representation of a vector uses finitely many elements.
...
Thus we have:
\[
\langle e_k, e_n \rangle = \begin{cases}
1, & k = n \\
0, & k \neq n
\end{cases}
\]
...
Then for every \(N \ge n \in \N\) we have
\[
c_n = \sum_{k=1}^N c_k \langle e_k, e_n \rangle
=
...
By identifying the locally integrable functions with the regular distributions they generate, we obtain an embedding of locally integrable functions into the distributions. 
...
\end{equation}
for all \(f \in \mathcal{H}\) and all \(\varphi \in \mathcal{H}'\).
\end{theorem}
...
\node[anchor=west,gray!50!black] at (0.6,0.3) { Vector Space \(\rightarrow\) linear combinations};
...
\node[anchor=west] at (\inpad+0.6,1.2) { \hyperref[def:norm]{Normed Space} \(\rightarrow\) length, distance, convergence};
...
\node[align=center, text=blue!60!black, ] at ($(Bctr)+(-2.4,0.8)$) {Banach Space\\{\tiny \(\rightarrow\) completeness}};
...
\node[align=center, text=ips!80!black, ] at ($(IPctr)+(2.4,0.8)$) {\hyperref[def:inner_product]{Inner Product Space}\\{\tiny\(\rightarrow\) angle, orthogonality}};
...
\end{scope}
\node[align=center, text=hilb] (HilbertLabel) at ($0.5*(Bctr)+0.5*(IPctr)+(0,1.1)$) {\hyperref[def:hilbert_space]{Hilbert Space}\\{\tiny \(\rightarrow\) \hyperref[theorem:orthogonal_projection]{projection}, \hyperref[def:orthonormal_basis]{ONB}, \hyperref[theorem:riesz_representation_theorem]{Riesz}}};
...
It is characterized by its mean $\mu(\vect{x})=\Exp[f(\vect{x})]$ and covariance
$k(\vect{x},\vect{x}')=\operatorname{Cov}(f(\vect{x}),f(\vect{x}'))$.
\end{definition}


\begin{theorem}[Connection to Gaussian processes]\label{ex:connection_gp_rkhs}
...
\).
Consider a Gaussian process $f \sim \mathcal{GP}(0,K)$ and set
\[
X := \{\vect{x}_1,\dots,\vect{x}_{n-1}\},
\qquad
\vect{y} := \bigl(f(\vect{x}_1),\dots,f(\vect{x}_{n-1})\bigr)^\top,
\qquad
...
\begin{enumerate}[label=(\roman *)]
  ...
  \]
  i.e.\ the power function evaluated at $\vect{x}_\star$ is exactly the conditional standard deviation (posterior uncertainty) of the Gaussian process at $\vect{x}_\star$ given the observations at $X$.
\end{enumerate}
\end{theorem}
...
% offset for measuring margin
\pgfmathsetmacro{\measureshift}{1mm}
...
\begin{tikzpicture}[
  ...
  \draw[important line] (\linestartx,\linestarty) -- (8,721/130) node[right, classPlus!60!black] {$\vect{w}^\top \vect{x} + b = 0$};
  ...
  \def\measureinset{0.0mm} % physical inward shift
