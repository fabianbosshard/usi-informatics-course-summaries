...
Let $n\in\N$ and let $\vect{x}\in\R^n$ be a column vector. For a scalar-valued function
$f:\R^n\to\R$ we use the \emph{Jacobian (numerator-layout)} convention
\[
\frac{\partial f}{\partial \vect{x}}\in\R^{1\times n}
\]
i.e.\ a row vector. (The column-gradient is obtained by transposition:
