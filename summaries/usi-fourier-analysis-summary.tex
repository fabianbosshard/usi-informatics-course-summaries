...
For every fixed \(\xi \in \R\), \eqref{eq:heat-equation-fourier} is a first-order ODE in \(t\) with solution given by
...
%   ∂ tt 2 u(t, x) = ∂ xx 2 u(t, x), t > 0, x ∈ R, u(0, x) = u 0 (x), x ∈ R,    ∂ t u(0, x) = v 0 (x), x ∈ R,
% for properly regular initial data u 0 , v 0 : R → C. By taking the Fourier transform, exactly as we did for the heat equation, we obtain the Cauchy problem
%   ∂ tt 2 uˆ(t, ξ ) = −4π 2 ξ 2 uˆ(t, ξ ), t > 0, ξ ∈ R, uˆ(0, ξ ) = u0 ⌃( ξ ), ∈ R,   ξ  ∂ t uˆ(0, ξ ) = v0 ⌃ (x), ξ ∈ R,
% (3.8) 42
% CHAPTER 3. FOURIER ANALYSIS OF PDES
% so we basically have to solve the ordinary differential equation y ′′ (t) = −4π 2 |ξ| 2 y(t), where y(t) = uˆ(t, ξ ), for every ξ ∈ R fixed. In the complex framework, solving this equation is simpler. We search for solutions in the form y(t) = e λt , and by substitution we obtain that it must be
% λ 2 + 4π 2 ξ 2 = 0,
% whose solutions are λ = ±2π |ξ| i. Thus, the solutions are in the form
% y(t) = uˆ(t, ξ ) = Ae −2π |ξ| it + Be 2π |ξ| it ,
% t > 0, for some A, B constant in t, i.e., A, B only depend on ξ . To find A and B, we differentiate and impose the initial conditions in (3.8):
% y(0) = A + B = u0 ⌃( ξ ), ′ { y (0) = 2πi |ξ| (−A + B) = v0 ⌃ ( ξ ),
% =⇒
%  A + B = u0 ⌃( ξ ), 1 −A + B = ( ξ ), v ⌃ 0  2πi |ξ|
% from which we obtain
%  A = 1 u 0 ( ξ ) − 1 2πi |ξ| v⌃ ( ξ ) 2 ( ⌃ 0 ) ,  B = 1 u 0 ( ξ ) + 1 2πi |ξ| v⌃ ( ξ ) ⌃ , 0  2 ( )
% whence
% 1 1 1 1 y(t) = uˆ(t, ξ ) = u 0 ( ξ ) − ( ξ ) e −2π |ξ| it + u 0 ( ξ ) + ( ξ ) e 2πi |ξ| t , ⌃ v ⌃ ⌃ v ⌃ 0 0 2 ( 2πi |ξ| ) 2 ( 2π |ξ| )
% and by associativity
% e2πi |ξ| t  +e −2πi |ξ| t −e−2πi |ξ| t  e2πi |ξ| t  sin(2πt ) uˆ(t, ξ ) = u0 ⌃( ξ ) + v0 ⌃ ( ξ ) = cos(2π |ξ| t)⌃u0 ( ξ ) + |ξ| v0 ⌃ ( ξ ) 2 2i(2π |ξ| ) 2π |ξ|
% sin(2πt ) = cos(2π ξ t)⌃u0 ( ξ ) + ξ v0 ⌃ ( ξ ), 2π ξ
% where we used that sin(·) and cos(·) are odd and even, respectively. Consequently,
% sin(2πt ξ ) u(t, x) = F −1 (cos(2π ξ t)⌃u0 )(x) + F −1 v0 ⌃ (x). ( 2π ξ )
% Using the inversion formula,
% ∞ ∞ e2πi ξ t +e−2πi ξ t  F −1 (cos(2π ξ t)⌃u0 )(x) = cos(2π ξ t)⌃u0 ( ξ )e 2πi ξ x d ξ = u0 ⌃( ξ )e 2πi ξ x d ξ ∫ −∞ (1.5) ∫ −∞ 2
% ∞ ∞ 1 1 = u⌃( ξ )e 2πi ξ (x+t) d ξ + u⌃( ξ )e 2πi ξ (x−t) d ξ 0 0 2 ∫ −∞ 2 ∫ −∞
% u 0 (x + t) + u 0 (x − t) = . (2.22) 2 3.3. THE WAVE EQUATION
% 43
% On the other hand,
% sin(2πt ξ ) 1 1 F −1 v0 ⌃ (x) = F −1 )v⌃ (x) = ∗ v 0 (x) F( 0 ( 2π ξ ) (Ex. 2.2.3) ( 2 χ [−t,t] ) (Thm. 2.3.2) 2 χ[−t,t] 
% ∞ t 1 1 = χ [−t,t] (y)v 0 (x − y)dy = v 0 (x − y)dy 2 ∫ −∞ 2 ∫ −t
% 1 x+t = v 0 (y ′ )dy ′ . (x − y = y ′ ) 2 ∫ x−t
% In conclusion, we retrieve d’Alembert’s formula:
% u0  (x + t) + u 0 (x − t) x+t 1 u(t, x) = + v 0 (y)dy. 2 2 ∫ x−t
...
To solve \eqref{eq:wave-equation}, take FT wrt the space variable \(x\). %, i.e., we fix \(t > 0\) and take the Fourier transform of \(u(t, \cdot)\).
It commutes with \(\partial_t\) and to the RHS we apply \autoref{thm:higher-derivatives}:
...
For every fixed \(\xi \in \R\), \eqref{eq:wave-equation-fourier} is a second-order ODE in \(t\) with solution given by
% If \(\xi \neq 0\), its general solution is
...
B(\xi) \sin(2\pi |\xi| t)
\end{equation}
where \(A(\xi)\) and \(B(\xi)\) are chosen to satisfy the initial conditions in \eqref{eq:wave-equation-fourier}.
Evaluating at \(t=0\) gives \(A(\xi) = \widehat{u_0}(\xi)\).
Differentiating \eqref{eq:wave-equation-fourier-general-solution} and evaluating at \(t=0\) yields
\[
\partial_t \hat{u}(0,\xi)
=
2\pi |\xi|\, B(\xi)
=
\widehat{v_0}(\xi)
\]
so that \(B(\xi) = \frac{\widehat{v_0}(\xi)}{2\pi |\xi|}\) for \(\xi \neq 0\).
Thus, for \(\xi \neq 0\),
...
\end{equation}
and for \(\xi = 0\) we have \(\partial_{tt}^2 \hat{u}(t,0)=0\), hence
\(
\hat{u}(t,0)=\widehat{u_0}(0)+t\,\widehat{v_0}(0)
\),
which is consistent with \eqref{eq:wave-equation-fourier-solution} by continuous continuation.
...
Using \autoref{eq:convolution-theorem} and \autoref{eq:fourier-transform-periodicity},
