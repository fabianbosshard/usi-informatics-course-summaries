...
\end{equation}
in \(L^1(\R)\).
In particular, \((\mathcal{F}^{-1} f)(x) = (\mathcal{F} f)(-x)\) and \((\mathcal{F}^4 f)(x) = f(x)\) in \(L^1(\R)\).
\end{corollary}
...
% We defined the Fourier transform F on L 1 (R) and L 2 (R). We showed that
% F : L 1 (R) → L ∞ (R) is bounded,
% and that
% F : L 2 (R) → L 2 (R)
% is a surjective isometry (in fact, a unitary operator).
% In particular, for p = 2 we have the complete characterization
% F(L 2 (R)) = L 2 (R).
% On the other hand, F(L 1 (R)) is far from being L ∞ (R). By the Riemann–Lebesgue Lemma we know that
% F(L 1 (R)) ⊆ C 0 (R) ∩ UC(R) ⊂ L ∞ (R),
% where UC(R) denotes the space of uniformly continuous functions on R and C 0 (R) ∩ UC(R) is much smaller that L ∞ (R): there exist functions in C 0 (R) ∩ UC(R) which are not the Fourier transform of any L 1 function. In other words,
% F(L 1 (R)) ⊂ C 0 (R) ∩ UC(R)
% (strict inclusion). Unlike the case p = 2, the image of L 1 (R) under the Fourier transform does not admit a simple characterization. More generally, the behavior of F on L p spaces is subtle. First of all, it can be proven that if f ∈ L p (R), 1 < p < 2, there exist (non-unique) f 1 ∈ L 1 (R) and f 2 ∈ L 2 (R) so that f = f 1 + f 2 . Hence, we may define the Fourier transform of f as f ˆ = f 1 ˆ + f 2 ˆ . Moreover, this definition does not depend on the choice of f 1 and f 2 . Concerning the boundedness of F on L p (R), 1 < p < 2, the following celebrated result holds. 36
% CHAPTER 2. FOURIER TRANSFORM ON L 1 (R)
% ′
% Theorem 2.6.1 (Hausdorff-Young). The Fourier transform F maps L p (R) to L p (R) for every 1 ≤ p ≤ 2. Moreover,
% d/2 p1/p  ∥ f ∥ p ′ ≤ (p ′ ) 1/p ′ ∥ f ∥ p , ( )
% f ∈ L p (R)
% (Babenko–Beckner inequality).
% The mapping
% ′ F : L p (R) → L p (R),
% 1 ≤ p ≤ 2,
% is surjective if and only if p = 2. In fact, F(L p (R)) admits an explicit characterization only in this case. Moreover, for every p > 2 there does always exist a function f ∈ L p (R) such that
% f ˆ ∈ / L q (R)
% for any 0 < q ≤ ∞.
% Even worse, one can find f ∈ L p (R) such that
% f ˆ ∈ / L loc 1 (R),
% where L loc 1 (R) denotes the space of measurable functions f : R → C satisfying
% | f(x) | dx < ∞ ∫ K
% for every compact K ⊆ R
% (locally integrable functions). In order to develop a Fourier theory for L p functions beyond the case p = 2, one is naturally led to the framework of distributions.
% 2.7 Final remarks
% The whole theory in this section can be formulated for multivariate functions f : R d → C, where
% if f ∈ L 1 (R d ),
% f( ˆ ξ ) = f(x)e −2πi ξ ·x dx. ∫ Rd 
% Here, ξ ·x = ∑ j=1 d xj  as
% ξj 
% is the standard inner product in R d . The lower dilation must be re-defined
% 1 f λ (x) = f(x/λ), λd 
% and Gaussians are replaced by multivariate Gaussians φ (x) = e −π x 2 | | .
% Generally speaking, the notation is slightly more difficult to digest in the multivariate setting, for instance we need to introduce the multi-index notation: if α = (α 1 , . . . , α d ) ∈ N d and x =
% (x 1 , . . . , x d ) ∈ R d ,
% x α = x 1 α 1 · . . . · x d α d ,
% and if f ∈ C k (R d ) and | α | = α 1 + . . . + α d ≤ k, we define
% ∂ | α | D α f(x) = f(x). ∂x 1 α 1 . . . ∂xd α d 
% Under this notation, formulas such that in Theorem 2.3.5, become
% (D α f) ∧ ( ξ ) = (2πi ξ ) α f( ˆ ξ ),
% where (2πi ξ ) α = (2πi) α |
% |
% ξ1 α 1 
% · ... · ξ d α d .
