...
\begin{digression}[Polya vector field]\label{dig:polya}
Given a complex function \(f = u + \iu v\), we can associate with it the vector field \(\vect{V} := [u, -v]^\top\) (\(\vect{V}_1\) from \autoref{rem:curve-integral}), which corresponds to the complex conjugate of \(f\) in the sense that \(\vect{V} = [\re(\overline{f}), \im(\overline{f})]^\top\).

The divergence and curl of \(\vect{V}\) are given by
\[
\begin{aligned}
...
\end{aligned}
\]
which vanish if \(f\) is holomorphic, because of \eqref{eq:cauchy-riemann}.
\end{digression}
...
\begin{remark}\label{rem:curve-integral}
...
\begin{digression}[continues=dig:polya]
In terms of the Polya vector field \(\vect{V}\), \eqref{eq:curve-integral} can be rewritten as
\[
\begin{aligned}
\int_C f(z) \dif z
&=
\int_\Gamma \vect{V} \cdot \vect{T} \dif l 
+
\iu \int_\Gamma \vect{V} \cdot \vect{N} \dif l \\
&= 
W_{\Gamma}(\vect{V}) + \iu F_{\Gamma}(\vect{V})
\end{aligned}
\]
where \(\vect{T}\) and \(\vect{N}\) are the unit tangent and (right) normal vectors along \(\Gamma\), respectively, \(\dif l\) is the arc length differential.
\(W_{\Gamma}(\vect{V})\) and \(F_{\Gamma}(\vect{V})\) denote the work and flux of \(\vect{V}\) along \(\Gamma\), respectively.
\end{digression}
...
% annulus as ONE filled shape (hole removed)
\path[fill=blue!20, draw, thick, even odd rule]
  (0,0) circle (\Rtwo)
  (0,0) circle (\Rone);
...
% radii using polar coordinates + node at endpoint
\draw[] (0,0) -- (40:\Rone)
  node[pos=1, above right, inner sep=1pt] {$R_1$};

\draw[] (0,0) -- (55:\Rtwo)
  node[pos=1, above right, inner sep=1pt] {$R_2$};

% label inside the annulus
\pgfmathsetmacro{\Rmid}{(\Rone+\Rtwo)/2}
\node at (235:\Rmid) {$D_{R_1,R_2}$};
