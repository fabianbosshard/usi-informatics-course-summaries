...
\end{equation}
hold at \((x,y)\), because the derivative needs to be a ``Drehstreckung'' (i.e., a composition of a rotation and a scaling), i.e. its Jacobian needs to be of the form \(\vect{f}' =  \left[\begin{smallmatrix} a & -b \\ b & a \end{smallmatrix}\right]\).
If a function is complex differentiable at every point of an simply connected open set \(D\), then it is called \emph{holomorphic} on \(D\).
...
\begin{itemize}
  ...
  \item \(f\) has a antiderivative \(F\) with \(\frac{\dif F}{\dif z} = f\) and \(\int_{z_{\text{start}}}^{z_{\text{end}}} f(z) \dif z = F(z_{\text{end}}) - F(z_{\text{start}})\)
  \qedhere
\end{itemize}
\end{theorem}
...
\]
where \(C\) is a positive oriented circle that does not contain \(z_0\).
...
So the above integral counts (up the the factor \(2 \pi \iu\)) how often \(C\) winds around \(z_0\).
...
It counts how often \(C\) winds around \(z_0\) in positive orientation.
We have \(\operatorname{Ind}_{-C}(z_0) = - \operatorname{Ind}_C(z_0)\).
...
No consider a closed curve \(C\) that circulates \(z_0\). % in positive orientation.
...
Another important consequence of \autoref{thm:cauchy-integral-formula} is that a inductive proof yields a formula for the \(n\)-th derivative of a holomorphic function \(f\),
...
% - Existenz beliebig hoher Ableitungen -> jede holomorphe Funktion kann in eine Potenzreihe entwickelt werde
% - konvergenz radius mindestens und höchstens bis zum rand des Holomorphiegebiets
% - Holomorphe Funktionen = (lokal) in Potenzreihen entwickelbare Funktionen
% - Identitätssatz
% - Beweis funktionalgleichung, inkl bsp sin^2 + cos^2 = 1
% - reellen Funktionen e x , sin x usw. nur auf eine Art holomorph nach ganz C fortsetzbar.(Deﬁnition der elementaren Funktionen mittels Potenzreihen war also „goldrichtig“)
% - maximumsprinzip
% - satz von Liouville
...
% &=
% \underbrace{\sum_{n=-1}^{-\infty} a_n (z - z_0)^n}_{\text{principal part}} + \underbrace{\sum_{n=0}^{\infty} a_n (z - z_0)^n}_{\text{regular part}}\\
= \underbrace{\sum_{n=1}^{\infty} a_{-n} (z - z_0)^{-n}}_{\text{principal part}} + \underbrace{\sum_{n=0}^{\infty} a_n (z - z_0)^n}_{\text{regular part}}
% \end{aligned}
\end{equation}
where \(z \in \C\) and \(a_n \in \C\) are the coefficients of the series.
It converges if both parts converge, so we apply the root test to both parts separately to obtain an outer and inner radius of convergence:
\begin{itemize}
\item principal part:
\[
\limsup_{n \to \infty} \sqrt[n]{|a_{-n} (z - z_0)^{-n}|} < 1 \quad \Leftrightarrow \quad |z - z_0| > \frac{1}{\limsup_{n \to \infty} |a_{-n}|^{1/n}} =: R_1
\]
\item regular part:
\[
\limsup_{n \to \infty} \sqrt[n]{|a_n (z - z_0)^n|} < 1 \quad \Leftrightarrow \quad |z - z_0| < \frac{1}{\limsup_{n \to \infty} |a_n|^{1/n}} =: R_2
\]
\end{itemize}
...
The inner and outer boundary need to be considered separately, as usual.
...
\end{equation}
is the Laurent series representation of a holomorphic function \(f\) on an annulus \(D_{R_1, R_2}\), then the coefficients \(a_n\) can be computed by the formula
...
For practical purposes, \eqref{eq:laurent-series-coefficients} is not very useful and the last step of dispaair.
On most cases of interest, the coefficients can be infered froma known poswer series or with the help of certain sum formulas (e.g. geometric).
