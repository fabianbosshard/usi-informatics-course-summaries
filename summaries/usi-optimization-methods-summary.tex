...
The latest version of this summary is available at \url{https://fabianbosshard.github.io/usi-informatics-course-summaries/}.
...
main components:
\begin{itemize}
	\item \emph{objective function} $f(\vect{x})$: function to be minimized or maximized
	\item \emph{variables} $\vect{x} = (x_1, \ldots, x_n) \in \mathbb{R}^n$: quantities of the problem that can be adjusted and need to be chosen \emph{optimally}
	\item \emph{constraint functions} $c_i(\vect{x})$: conditions that limit the possbile values of $\vect{x}$. they can be \emph{equality} or \emph{inequality} constraints
	\item \emph{feasible region}: set of all variables $\vect{x}$ satisfying all constraints
\end{itemize}
